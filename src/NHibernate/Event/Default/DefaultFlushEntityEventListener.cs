...
namespace NHibernate.Event.Default
{
	...
	public class DefaultFlushEntityEventListener : IFlushEntityEventListener
	{
		...
		public virtual async Task OnFlushEntity(FlushEntityEvent @event)
		{
			...
			object[] values = await GetValues(entity, entry, entityMode, mightBeDirty, session).ConfigureAwait(false);

			@event.PropertyValues = values;

			//TODO: avoid this for non-new instances where mightBeDirty==false
			bool substitute = await WrapCollections(session, persister, types, values).ConfigureAwait(false);

			if (await IsUpdateNecessary(@event, mightBeDirty).ConfigureAwait(false))
			{
				substitute = await ScheduleUpdate(@event).ConfigureAwait(false) || substitute;
			}
			...
			if (status != Status.Deleted)
			{
				...
				{
					await new FlushVisitor(session, entity).ProcessEntityPropertyValues(values, types).ConfigureAwait(false);
				}
			}
		}
		...
		private async Task<object[]> GetValues(object entity, EntityEntry entry, EntityMode entityMode, bool mightBeDirty, ISessionImplementor session)
		{
			...
			else
			{
				...
				await CheckNaturalId(persister, entry, values, loadedState, entityMode, session).ConfigureAwait(false);
			}
			return values;
		}
				...
				for (int i = 0; i < props.Length; i++)
				{
					...
					if (!updateable[prop])
					{
						...
						if (loaded == null)
						{
							if (snapshot == null)
							{
								snapshot = await session.PersistenceContext.GetNaturalIdSnapshot(entry.Id, persister).ConfigureAwait(false);
							}
		...
		private async Task<bool> IsUpdateNecessary(FlushEntityEvent @event, bool mightBeDirty)
		{
			...
			if (mightBeDirty || status == Status.Deleted)
			{
				...
				await DirtyCheck(@event).ConfigureAwait(false);
				if (await IsUpdateNecessary(@event).ConfigureAwait(false))
				{
					return true;
				}
				else
			...
			{
				return await HasDirtyCollections(@event, @event.EntityEntry.Persister, status).ConfigureAwait(false);
			}
		}
		...
		private async Task<bool> ScheduleUpdate(FlushEntityEvent @event)
		{
			...
			if (!entry.IsBeingReplicated)
			{
				...
				intercepted = await HandleInterception(@event).ConfigureAwait(false);
			}
			else
			{
				intercepted = false;
		...
		protected virtual async Task<bool> HandleInterception(FlushEntityEvent @event)
		{
			...
			if (intercepted && @event.DirtyCheckPossible && !@event.DirtyCheckHandledByInterceptor)
			{
				...
				{
					dirtyProperties = await persister.FindModified(@event.DatabaseSnapshot, values, entity, session).ConfigureAwait(false);
				}
				else
				{
					dirtyProperties = await persister.FindDirty(values, entry.LoadedState, entity, session).ConfigureAwait(false);
				}
		...
		protected async Task<bool> IsUpdateNecessary(FlushEntityEvent @event)
		{
			...
			else
			{
				...
				{
					return await HasDirtyCollections(@event, persister, status).ConfigureAwait(false);
				}
			}
		}
		...
		private async Task<bool> HasDirtyCollections(FlushEntityEvent @event, IEntityPersister persister, Status status)
		{
			if (IsCollectionDirtyCheckNecessary(persister, status))
			{
				...
				await visitor.ProcessEntityPropertyValues(@event.PropertyValues, persister.PropertyTypes).ConfigureAwait(false);
		...
		protected virtual async Task DirtyCheck(FlushEntityEvent @event)
		{
			...
			if (dirtyProperties == null)
			{
				...
				if (!cannotDirtyCheck)
				{
					...
					dirtyProperties = await persister.FindDirty(values, loadedState, entity, session).ConfigureAwait(false);
				}
				else if (entry.Status == Status.Deleted && !@event.EntityEntry.IsModifiableEntity())
				{
					...
					dirtyProperties = await persister.FindDirty(entry.DeletedState, currentState, entity, session).ConfigureAwait(false);
					cannotDirtyCheck = false;
				}
				else
				{
					// dirty check against the database snapshot, if possible/necessary
					object[] databaseSnapshot = await GetDatabaseSnapshot(session, persister, id).ConfigureAwait(false);
					if (databaseSnapshot != null)
					{
						dirtyProperties = await persister.FindModified(databaseSnapshot, values, entity, session).ConfigureAwait(false);
						cannotDirtyCheck = false;
		...
		private async Task<object[]> GetDatabaseSnapshot(ISessionImplementor session, IEntityPersister persister, object id)
		{
			...
			{
				object[] snapshot = await session.PersistenceContext.GetDatabaseSnapshot(id, persister).ConfigureAwait(false);
				if (snapshot == null)
				{
			...
			else
			{
				...
				return await session.PersistenceContext.GetCachedDatabaseSnapshot(entityKey).ConfigureAwait(false);
			}
		}
