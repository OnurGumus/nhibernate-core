		...
		internal override Task<object> ProcessCollection(object collection, CollectionType type)
		{
			...
			else
			{
					...
					if (persistentCollection.SetCurrentSession(session))
					{
						if (IsOwnerUnchanged(persistentCollection, persister, ExtractCollectionKeyFromOwner(persister)))
						{
							...
							if (persistentCollection.IsDirty)
							{
								...
								return TaskHelper.FromException<object>(new HibernateException("reassociated object has dirty collection: " + persistentCollection.Role));
							}
						...
						else
						{
							...
							return TaskHelper.FromException<object>(new HibernateException("reassociated object has dirty collection reference: " + persistentCollection.Role));
						}
					}
					else
					{
				...
				else
				{
					...
					return TaskHelper.FromException<object>(new HibernateException("reassociated object has dirty collection reference (or an array)"));
				}
			}
