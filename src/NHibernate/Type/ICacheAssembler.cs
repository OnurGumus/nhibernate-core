...
namespace NHibernate.Type
{
	public interface ICacheAssembler
	{
		...
		Task<object> Disassemble(object value, ISessionImplementor session, object owner);
		...
		Task<object> Assemble(object cached, ISessionImplementor session, object owner);

		/// <summary>
		/// Called before assembling a query result set from the query cache, to allow batch fetching
		/// of entities missing from the second-level cache.
		/// </summary>
		Task BeforeAssemble(object cached, ISessionImplementor session);
	}
}