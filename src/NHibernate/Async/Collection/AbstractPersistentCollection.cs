...
namespace NHibernate.Collection
{
	...
	public abstract partial class AbstractPersistentCollection : IPersistentCollection
	{
			...
			if (!initialized)
			{
				...
				else
				{
					...
					if (persister.IsExtraLazy)
					{
						...
						cachedSize = await (persister.GetSizeAsync(entry.LoadedKey, session)).ConfigureAwait(false);
						return true;
					}
				}
			}
		...
		protected virtual async Task<bool?> ReadIndexExistenceAsync(object index)
		{
			if (!initialized)
			{
				...
				if (persister.IsExtraLazy)
				{
					...
					return await (persister.IndexExistsAsync(entry.LoadedKey, index, session)).ConfigureAwait(false);
				}
			}
			await (ReadAsync()).ConfigureAwait(false);
			return null;
		...
		protected virtual async Task<bool?> ReadElementExistenceAsync(object element)
		{
			if (!initialized)
			{
				...
				if (persister.IsExtraLazy)
				{
					...
					return await (persister.ElementExistsAsync(entry.LoadedKey, element, session)).ConfigureAwait(false);
				}
			}
			await (ReadAsync()).ConfigureAwait(false);
			return null;
		...
		protected virtual async Task<object> ReadElementByIndexAsync(object index)
		{
			if (!initialized)
			{
				...
				if (persister.IsExtraLazy)
				{
					...
					var elementByIndex = await (persister.GetElementByIndexAsync(entry.LoadedKey, index, session, owner)).ConfigureAwait(false);
		...
		{
			await (InitializeAsync(true)).ConfigureAwait(false);
			Dirty();
		}
