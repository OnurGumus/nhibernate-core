...
namespace NHibernate.SqlCommand
{
	...
	public partial interface ISqlCommand
	{
		...
		/// </remarks>
		Task BindAsync(IDbCommand command, IList<Parameter> commandQueryParametersList, int singleSqlParametersOffset, ISessionImplementor session);
		/// <summary>
		/// Bind the appropriate value into the given command.
		/// </summary>
		/// <param name = "command">The command into which the value should be bound.</param>
		...
		/// </remarks>
		Task BindAsync(IDbCommand command, ISessionImplementor session);
	}
