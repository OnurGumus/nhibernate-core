	...
	public class FixtureAsync : BugTestCase
	{
		...
		public async Task ConditionalGroupKeyFromArrayAccessAsync()
		{
			...
			using (var transaction = session.BeginTransaction())
			{
				...
				Assert.That(await (baseQuery.SumAsync(x => x.TimeInHours, CancellationToken.None)), Is.EqualTo(78));
				...
				var results = (await (query.ToListAsync(CancellationToken.None))).OrderBy(x => (int)((object[])x[0])[0]);
		...
		public async Task ConditionalInComplexGroupKeyFromSubqueryArrayAccessAsync()
		{
			...
			using (var transaction = session.BeginTransaction())
			{
				...
				var results = (await (query.ToListAsync(CancellationToken.None))).OrderBy(x => (int)((object[])x[0])[0]).ThenBy(x => (string)((object[])x[0])[1]);
