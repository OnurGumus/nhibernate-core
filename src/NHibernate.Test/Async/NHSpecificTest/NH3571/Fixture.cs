		...
		public void DifferentKeyInDynamicComponentDictionaryReturnsDifferentExpressionKeys()
		{
			using (var session = OpenSession())
			{
				...
				Expression<Func<IEnumerable>> key1 = () => (
					from a in session.Query<Product>()where (string)a.Details.Properties["Name"] == "val"
					select a);
				Expression<Func<IEnumerable>> key2 = () => (
					from a in session.Query<Product>()where (string)a.Details.Properties["Description"] == "val"
					select a);
