		...
		public async Task DatePropertyShouldBeStoredWithoutTimePartAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				...
				var l = await (session.CreateSQLQuery("SELECT BirthDate AS bd FROM DomainClass")
					.AddScalar("bd",NHibernateUtil.DateTime).ListAsync(CancellationToken.None));
			...
			using (ITransaction tx = session.BeginTransaction())
			{
				await (session.CreateQuery("delete from DomainClass").ExecuteUpdateAsync(CancellationToken.None));
				await (tx.CommitAsync(CancellationToken.None));
			}
		}
	}
