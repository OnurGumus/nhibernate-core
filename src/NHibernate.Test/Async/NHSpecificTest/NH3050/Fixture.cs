...
namespace NHibernate.Test.NHSpecificTest.NH3050
{
	...
	public partial class FixtureAsync : BugTestCaseAsync
	{
		...
		public async Task TestAsync()
		{
			...
			queryPlanCacheType.GetField("planCache", BindingFlags.Instance | BindingFlags.NonPublic).SetValue(sessions.QueryPlanCache, cache);
			...
			IEnumerable<int> personIds = new List<int>{1};
			...
			try
			{
				...
				System.Action queryExecutor = () =>
				{
					...
					try
					{
						...
						{
							(
								from person in session.Query<Person>()where personIds.Contains(person.Id)select person).ToList();
						}
				...
				;
				await ((
					from person in session.Query<Person>()where personIds.Contains(person.Id)select person).ToListAsync());
