	...
	public partial class Fixture : BugTestCase
	{
		...
		public async Task TestHqlStatementsContainingSqlServerDataTypeKeywordsAsync()
		{
			using (ISession session = Sfi.OpenSession())
				using (ITransaction transaction = session.BeginTransaction())
				{
					var students = session.CreateQuery("from Student where length(convert(varbinary, FullName)) = 28").List<Student>();
					Assert.That(students.Count, Is.EqualTo(1));
					students = session.CreateQuery("from Student where length(convert(varbinary(256), FullName)) = 28").List<Student>();
					Assert.That(students.Count, Is.EqualTo(1));
					students = session.CreateQuery("from Student where convert(int, 1) = 1").List<Student>();
					Assert.That(students.Count, Is.EqualTo(1));
					await (transaction.CommitAsync());
				}
		}
