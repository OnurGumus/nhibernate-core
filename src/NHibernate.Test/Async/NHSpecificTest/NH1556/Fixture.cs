	...
	public partial class FixtureAsync : BugTestCaseAsync
	{
		...
		protected override async Task OnSetUpAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				...
				{
					await (session.SaveAsync(prozac));
					await (session.SaveAsync(warfarin));
					await (session.SaveAsync(patient));
					await (session.SaveAsync(prozacClaim1));
					await (session.SaveAsync(prozacClaim2));
					await (session.SaveAsync(warfarinClaim1));
		...
		protected override async Task OnTearDownAsync()
		{
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					await (session.DeleteAsync("from Claim"));
					await (session.DeleteAsync("from Patient"));
					await (session.DeleteAsync("from ProductIdentifier"));
		...
		public async Task CanOrderByAggregateAsync()
		{
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					...
					IList list = await (session.CreateQuery(@"select p.Id, p.ProductName, max(c.LastFilled), count(c.Id)
from Claim as c
join c.ProductIdentifier.Product as p
where c.Patient = :patient
group by p.Id, p.ProductName
order by max(c.LastFilled) asc, p.ProductName").SetParameter("patient", loadedPatient).SetFirstResult(0).SetMaxResults(2).ListAsync());
