...
namespace NHibernate.Test.NHSpecificTest.NH3383
{
	...
	public partial class ByCodeFixtureAsync : TestCaseMappingByCodeAsync
	{
		...
		{
			CascadeStyle startingCascadeStyle = new CascadeStyle.MultipleCascadeStyle(new[]{CascadeStyle.Delete, CascadeStyle.Lock});
			CascadeStyle deserializedCascadeStyle;
			using (var configMemoryStream = new MemoryStream())
			{
				var formatter = new BinaryFormatter();
			...
			Assert.That(deserializedCascadeStyle.ToString(), Is.EqualTo("[NHibernate.Engine.CascadeStyle+DeleteCascadeStyle,NHibernate.Engine.CascadeStyle+LockCascadeStyle]"));
		}
		...
		public void DeserializedPropertyMapping_CascadeStyleNotYetInitializedOnDeserialization_RefersToSameCascadeStyle()
		{
			...
			using (MemoryStream configMemoryStream = new MemoryStream())
			{
				...
				formatter.Serialize(configMemoryStream, classMapping);
				configMemoryStream.Position = 0;
				var secondAppDomain = AppDomain.CreateDomain("SecondAppDomain", null, AppDomain.CurrentDomain.SetupInformation);
				try
				{
					var helper = (AppDomainHelper)secondAppDomain.CreateInstanceAndUnwrap(Assembly.GetExecutingAssembly().FullName, typeof (AppDomainHelper).FullName);
