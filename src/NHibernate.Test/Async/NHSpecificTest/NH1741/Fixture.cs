...
namespace NHibernate.Test.NHSpecificTest.NH1741
{
	...
	public partial class FixtureAsync : BugTestCaseAsync
	{
		...
		{
			var dq = new DetachedNamedQueryCrack(QueryName);
			dq.SetCacheable(false).SetCacheRegion("another region").SetReadOnly(false).SetTimeout(20).SetCacheMode(CacheMode.Refresh).SetFetchSize(22).SetComment("another comment").SetFlushMode(FlushMode.Commit);
			ISession s = sessions.OpenSession();
			dq.GetExecutableQuery(s);
			s.Close();
			Assert.That(!dq.Cacheable);
