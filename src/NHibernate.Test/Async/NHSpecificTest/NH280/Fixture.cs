	...
	public class FixtureAsync : TestCase
	{
		...
		public async Task ConstInSelectAsync()
		{
			using (ISession s= OpenSession())
			{
				...
				IList l = await (s.CreateQuery("select 'TextConst', 123, 123.5, .5 from Foo").ListAsync(CancellationToken.None));
				...
				l = await (s.CreateQuery("select 123, f from Foo f").ListAsync(CancellationToken.None));
				result = l[0] as IList;
				...
				l = await (s.CreateQuery("select 123, f.Description from Foo f").ListAsync(CancellationToken.None));
				result = l[0] as IList;
				Assert.AreEqual(123, result[0]);
		...
		public void TokenUnknowAsync()
		{
			using (ISession s = OpenSession())
			{
				Assert.ThrowsAsync<QueryException>(() =>s.CreateQuery("select 123, notRecognized, f.Description from Foo f").ListAsync(CancellationToken.None));
			}
		}
	}
}
