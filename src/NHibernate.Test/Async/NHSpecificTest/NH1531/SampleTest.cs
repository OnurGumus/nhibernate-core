...
namespace NHibernate.Test.NHSpecificTest.NH1531
{
	...
	public class SampleTestAsync : BugTestCase
	{
		...
		public async Task ReparentingShouldNotFailAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				var parent1 = await (session.GetAsync<Parent>(1, CancellationToken.None));
				var parent2 = await (session.GetAsync<Parent>(2, CancellationToken.None));
				...
				await (session.SaveOrUpdateAsync(parent1, CancellationToken.None));
				await (session.SaveOrUpdateAsync(parent2, CancellationToken.None));
			...
			using (ISession session = OpenSession())
			{
				...
				var l = await (session.CreateQuery("from Child").ListAsync(CancellationToken.None));
				Assert.That(l.Count, Is.EqualTo(1));
			}
			await (CleanDbAsync(CancellationToken.None));
		}
