	...
	public partial class FixtureAsync : BugTestCaseAsync
	{
		...
		public async Task _1_Querying_BasedOnFourthLevelExistence_WithIsAndCasting_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Any(second => second.ThirdLevel is SpecificThirdLevel && ((SpecificThirdLevel)second.ThirdLevel).FourthLevels.Any())).ToListAsync());
		...
		public async Task _2_Querying_BasedOnFourthLevelExistence_WithSelectAndOfType_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Select(second => second.ThirdLevel).OfType<SpecificThirdLevel>().Any(specificThird => specificThird.FourthLevels.Any())).ToListAsync());
					Assert.AreEqual(expected.Count, result.Count);
					Assert.IsTrue(result.All(f => f.SecondLevels.Any(s => ((SpecificThirdLevel)s.ThirdLevel).FourthLevels.Any())));
				}
			}
		...
		public async Task _3_Querying_BasedOnFourthLevelProperty_WithIsAndCasting_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Any(second => second.ThirdLevel is SpecificThirdLevel && ((SpecificThirdLevel)second.ThirdLevel).FourthLevels.Any(fourth => fourth.SomeString == "first"))).ToListAsync());
					Assert.AreEqual(expected.Count, result.Count);
		...
		public async Task _4_Querying_BasedOnFourthLevelProperty_WithSelectAndOfType_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Select(second => second.ThirdLevel).OfType<SpecificThirdLevel>().Any(specificThird => specificThird.FourthLevels.Any(fourth => fourth.SomeString == "first"))).ToListAsync());
					Assert.AreEqual(expected.Count, result.Count);
					Assert.IsTrue(result.All(f => f.SecondLevels.Any(s => ((SpecificThirdLevel)s.ThirdLevel).FourthLevels.Any(fo => fo.SomeString == "first"))));
				}
			}
		...
		public async Task _5_Querying_BasedOnFourthLevelExistence_ByUsingSpecificThirdLevelProperty_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Any(second => second.SpecificThirdLevel.FourthLevels.Any())).ToListAsync());
					Assert.AreEqual(expected.Count, result.Count);
		...
		public async Task _6_Querying_BasedOnFourthLevelProperty_ByUsingSpecificThirdLevelProperty_ShouldReturnSameEntitiesAsLinqToObjectsAsync()
		{
			...
			using (ISession session = OpenSession())
			{
				using (ITransaction tx = session.BeginTransaction())
				{
					var result = await (session.Query<FirstLevel>().Where(first => first.SecondLevels.Any(second => second.SpecificThirdLevel.FourthLevels.Any(fourth => fourth.SomeString == "first"))).ToListAsync());
					Assert.AreEqual(expected.Count, result.Count);
