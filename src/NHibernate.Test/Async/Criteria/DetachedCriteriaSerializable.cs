...
namespace NHibernate.Test.Criteria
{
	...
	public partial class DetachedCriteriaSerializableAsync : TestCaseAsync
	{
		...
		{
			ICriterion c = Expression.Or(Expression.Eq("Name", "Ralph"), Expression.Eq("Name", "Gavin"));
			NHAssert.IsSerializable(c);
			c = Expression.And(Expression.Gt("StudentNumber", 1), Expression.Lt("StudentNumber", 10));
			NHAssert.IsSerializable(c);
		}
		...
		{
			ICriterion c = Expression.Conjunction().Add(Expression.Eq("Name", "Ralph")).Add(Expression.Eq("StudentNumber", "1"));
			NHAssert.IsSerializable(c);
			c = Expression.Disjunction().Add(Expression.Eq("Name", "Ralph")).Add(Expression.Eq("Name", "Gavin"));
			NHAssert.IsSerializable(c);
		}
		...
		{
			IProjection p = Projections.SqlProjection("COUNT(*)", new string[]{"tStudent"}, new IType[]{NHibernateUtil.Int32});
			NHAssert.IsSerializable(p);
			p = Projections.SqlGroupProjection("COUNT({alias}.studentId), {alias}.preferredCourseCode", "{alias}.preferredCourseCode", new string[]{"studentsOfCourse", "CourseCode"}, new IType[]{NHibernateUtil.Int32, NHibernateUtil.Int32});
			NHAssert.IsSerializable(p);
		}
