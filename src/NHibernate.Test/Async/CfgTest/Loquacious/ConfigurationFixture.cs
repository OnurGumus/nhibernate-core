...
namespace NHibernate.Test.CfgTest.Loquacious
{
	...
	public partial class ConfigurationFixtureAsync
	{
		[Test]
		public void CompleteConfiguration()
		{
			...
			cfg.SessionFactory().Named("SomeName").Caching.Through<HashtableCacheProvider>().PrefixingRegionsWith("xyz").Queries.Through<StandardQueryCache>().UsingMinimalPuts().WithDefaultExpiration(15).GeneratingCollections.Through<DefaultCollectionTypeFactory>().Proxy.DisableValidation().Through<DefaultProxyFactoryFactory>().ParsingHqlThrough<ASTQueryTranslatorFactory>().Mapping.UsingDefaultCatalog("MyCatalog").UsingDefaultSchema("MySche").Integrate.Using<MsSql2000Dialect>().AutoQuoteKeywords().EnableLogFormattedSql().BatchingQueries.Through<SqlClientBatchingBatcherFactory>().Each(15).Connected.Through<DebugConnectionProvider>().By<SqlClientDriver>().Releasing(ConnectionReleaseMode.AfterTransaction).With(IsolationLevel.ReadCommitted).Using(connectionString).CreateCommands.AutoCommentingSql().ConvertingExceptionsThrough<SQLStateConverter>().Preparing().WithTimeout(10).WithMaximumDepthOfOuterJoinFetching(11).WithHqlToSqlSubstitutions("true 1, false 0, yes 'Y', no 'N'").Schema.Validating();
		...
		public void UseDbConfigurationStringBuilder()
		{
			...
			var cfg = new Configuration();
			cfg.SessionFactory().Proxy.Through<DefaultProxyFactoryFactory>().Integrate.Using<MsSql2005Dialect>().Connected.Using(new SqlConnectionStringBuilder{DataSource = "(local)", InitialCatalog = "nhibernate", IntegratedSecurity = true});
			Assert.That(cfg.Properties[Environment.ProxyFactoryFactoryClass], Is.EqualTo(typeof (DefaultProxyFactoryFactory).AssemblyQualifiedName));
		...
		{
			var cfg = new Configuration();
			cfg.SessionFactory().Integrate.Connected.ByAppConfing("MyName");
		...
		{
			var cfg = new Configuration();
			cfg.Configure("TestEnbeddedConfig.cfg.xml").SetDefaultAssembly("NHibernate.DomainModel").SetDefaultNamespace("NHibernate.DomainModel").SessionFactory().ParsingLinqThrough<NHibernate.Test.CfgTest.ConfigurationFixtureAsync.SampleQueryProvider>();
			using (var sessionFactory = cfg.BuildSessionFactory())
			{
				using (var session = sessionFactory.OpenSession())
				{
					var query = session.Query<NHibernate.DomainModel.A>();
					Assert.IsInstanceOf(typeof (NHibernate.Test.CfgTest.ConfigurationFixtureAsync.SampleQueryProvider), query.Provider);
				}
			}
		}
	}
