...
namespace NHibernate.Test.CfgTest
{
	...
	public partial class ConfigurationFixtureAsync
	{
		...
		public void ReadCfgXmlFromDefaultFile()
		{
			...
			Assert.IsFalse(cfg.Properties.ContainsKey(Environment.PrepareSql), "Our default conf should not include override the possible Dialect default configuration.");
			...
			Assert.AreEqual("Server=localhost;initial catalog=nhibernate;User Id=;Password=", cfg.Properties[Environment.ConnectionString]);
		}
...
<hibernate-configuration xmlns='urn:nhibernate-configuration-2.2'>
	<session-factory name='NHibernate.Test'>
		...
		[Test]
		public Task CacheConfigurationForUnmappedClassAsync()
		{
			try
			{
		...
		[Test]
		public Task CacheConfigurationForUnmappedCollectionAsync()
		{
			try
			{
				string cfgString = @"<?xml version='1.0' encoding='utf-8' ?> 
		...
		[Test]
		public Task NoSessionFactoriesInConfigurationAsync()
		{
			try
			{
  ...
  <event type='flush'>
    <listener class='NHibernate.Event.Default.DefaultFlushEventListener, NHibernate'/>
		...
		public void NH2890Standard()
		{
			...
			cfg.Configure("TestEnbeddedConfig.cfg.xml").LinqQueryProvider<SampleQueryProvider>().SetDefaultAssembly("NHibernate.DomainModel").SetDefaultNamespace("NHibernate.DomainModel");
		...
		{
			var cfg = new Configuration();
			cfg.Configure("TestEnbeddedConfig.cfg.xml").SetDefaultAssembly("NHibernate.DomainModel").SetDefaultNamespace("NHibernate.DomainModel");
			using (var sessionFactory = cfg.BuildSessionFactory())
			{
				using (var session = sessionFactory.OpenSession())
				{
