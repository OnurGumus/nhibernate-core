...
namespace NHibernate.Test.Linq
{
	...
	public partial class AggregateTestsAsync : LinqTestCaseAsync
	{
		[Test]
		public void AggregateWithStartsWith()
		{
			var query = (
				...
				where c.CustomerId.StartsWith("A")select c.CustomerId).Aggregate(new StringBuilder(), (sb, id) => sb.Append(id).Append(","));
		...
		public void AggregateWithEndsWith()
		{
			var query = (
				from c in db.Customers
				where c.CustomerId.EndsWith("TH")select c.CustomerId).Aggregate(new StringBuilder(), (sb, id) => sb.Append(id).Append(","));
			Console.WriteLine(query);
		...
		public void AggregateWithContains()
		{
			var query = (
				from c in db.Customers
				where c.CustomerId.Contains("CH")select c.CustomerId).Aggregate(new StringBuilder(), (sb, id) => sb.Append(id).Append(","));
			Console.WriteLine(query);
		...
		public void AggregateWithEquals()
		{
			var query = (
				from c in db.Customers
				where c.CustomerId.Equals("ALFKI") || c.CustomerId.Equals("ANATR") || c.CustomerId.Equals("ANTON")select c.CustomerId).Aggregate((prev, next) => (prev + "," + next));
			Console.WriteLine(query);
		...
		public void AggregateWithNotStartsWith()
		{
			var query = (
				from c in db.Customers
				where c.CustomerId.StartsWith("A") && !c.CustomerId.StartsWith("AN")select c.CustomerId).Aggregate(new StringBuilder(), (sb, id) => sb.Append(id).Append(","));
			Console.WriteLine(query);
		...
		public void AggregateWithMonthFunction()
		{
			...
			var query = (
				...
				select e.FirstName).Aggregate(new StringBuilder(), (sb, name) => sb.Length > 0 ? sb.Append(", ").Append(name) : sb.Append(name));
		...
		public void AggregateWithBeforeYearFunction()
		{
			...
			var query = (
				...
				select e.FirstName.ToUpper()).Aggregate(new StringBuilder(), (sb, name) => sb.Length > 0 ? sb.Append(", ").Append(name) : sb.Append(name));
		...
		public void AggregateWithUpperAndLowerFunctions()
		{
			...
			var query = (
				...
				select new
				{
				First = e.FirstName.ToUpper(), Last = e.LastName.ToLower()}
