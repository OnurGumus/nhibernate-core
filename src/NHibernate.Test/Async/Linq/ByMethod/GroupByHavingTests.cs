	...
	public class GroupByHavingTestsAsync : LinqTestCase
	{
		...
		public async Task HavingCountSelectCountAsync()
		{
			var list = await ((from o in db.Orders
						...
						into g
						where g.Count() > 4
						select g.Count()).ToListAsync(CancellationToken.None));
		...
		public async Task HavingCountSelectTupleKeyCountAsync()
		{
			var list = await ((from o in db.Orders
						...
						into g
						where g.Count() > 4
						select new {g.Key, Count = g.Count()}).ToListAsync(CancellationToken.None));
		...
		public async Task HavingCountSelectTupleKeyCountOfOrdersAsync()
		{
			var list = await ((from o in db.Orders
						...
						into g
						where g.Count() > 4
						select new {g.Key, Count = g.Select(x => x.OrderId).Count()}).ToListAsync(CancellationToken.None));
