	...
	public partial class AnyTestsAsync : LinqTestCaseAsync
	{
		...
		public void NestedAny()
		{
			var test = (
				...
				where c.ContactName == "Bob" && (c.CompanyName == "NormalooCorp" || c.Orders.Any(o => o.OrderLines.Any(ol => ol.Discount < 20 && ol.Discount >= 10)))select c).ToList();
		...
		{
			var result = db.Orders.Any(p => p.OrderLines.Count == 0);
