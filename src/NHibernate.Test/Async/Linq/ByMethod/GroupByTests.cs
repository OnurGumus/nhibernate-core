...
namespace NHibernate.Test.Linq.ByMethod
{
	...
	public partial class GroupByTestsAsync : LinqTestCaseAsync
	{
		...
		{
			var orderCounts = db.Orders.GroupBy(o => new
			{
		...
		{
			var orders = db.Orders.GroupBy(o => new
			{
			o.Customer, o.Employee
			}
		...
		{
			//NH-2452
			var result = db.Products.GroupBy(i => i.UnitPrice).OrderBy(g => g.Key).Select(g => new
			{
		...
		{
			//NH-2452
			var result = db.Products.GroupBy(i => i.UnitPrice).Select(g => new
			{
			UnitPrice = g.Max(i => i.UnitPrice), TotalUnitsInStock = g.Sum(i => i.UnitsInStock)}
		...
		{
			// NH-2560
			var orderCounts = db.Orders.GroupBy(o => o.Customer.CustomerId).Select(g => new
			{
		...
		public void SingleKeyPropertyGroupAndOrderByCountBeforeProjection()
		{
			...
			var orderCounts = db.Orders.GroupBy(o => o.Customer.CustomerId).OrderByDescending(g => g.Count()).Select(g => new
			{
			CustomerId = g.Key, OrderCount = g.Count()}
		...
		public void SingleKeyPropertyGroupWithOrderByCount()
		{
			...
			try
			{
				var result = db.Orders.GroupBy(o => o.Customer).OrderByDescending(g => g.Count()) // it seems like there we should do order on client-side
		...
		public void SingleKeyPropertyGroupByEntityAndSelectEntity()
		{
			...
			try
			{
				var orderCounts = db.Orders.GroupBy(o => o.Customer).Select(g => new
				{
				Customer = g.Key, OrderCount = g.Count()}
		...
		{
			//NH-2452
			var result = db.Products.GroupBy(p => p.UnitPrice).Select(g => new
			{
			UnitPrice = g.Max(i => i.UnitPrice), TotalUnitsInStock = g.Sum(i => i.UnitsInStock)}

			).OrderBy(x => x.TotalUnitsInStock).ToList();
			Assert.That(result.Count, Is.EqualTo(62));
		...
		public void SingleKeyGroupAndOrderByKeyProjection()
		{
			...
			var result = db.Products.GroupBy(i => i.UnitPrice).Select(g => new
			{
			UnitPrice = g.Key, TotalUnitsInStock = g.Sum(i => i.UnitsInStock)}
		...
		public void GroupByWithAndAlsoContainsInWhereClause()
		{
			...
			var result = db.Products.Where(x => x.Discontinued == true && collection.Contains(x.Supplier)).GroupBy(x => x.UnitPrice).Select(x => new
			{
			x.Key, Count = x.Count()}
		...
		public void GroupByWithContainsInWhereClause()
		{
			//NH-3032
			var collection = db.Products.Select(x => x.Supplier).ToList();
			var result = db.Products.Where(x => collection.Contains(x.Supplier)).GroupBy(x => x.UnitPrice).Select(x => new
			{
			x.Key, Count = x.Count()}
		...
		[Test]
		public Task SelectTupleKeyCountOfOrderLinesAsync()
		{
			try
			{
		...
		public void GroupByAndTake2()
		{
			...
			var results = (
				...
				group o by o.Customer into g
					select g.Key.CustomerId).OrderBy(customerId => customerId).Skip(10).Take(10).ToList();
			Assert.That(results.Count, Is.EqualTo(10));
		}

		[Test]
		public Task GroupByAndAllAsync()
		{
			try
			{
		...
		{
			//NH-3180
			var result = db.Products.GroupBy(x => x.UnitPrice).Select(x => new
			{
			x.Key, Count = x.Count()}
		...
		public void SelectSingleElementFromProductsGroupedByUnitPrice()
		{
			...
			).Where(x => x.Key == 263.5M).OrderByDescending(x => x.Key).Single();
			Assert.That(result.Key, Is.EqualTo(263.5M));
			Assert.That(result.Count, Is.EqualTo(1));
		}
		...
		public void SelectSingleOrDefaultElementFromProductsGroupedByUnitPrice()
		{
			...
			).Where(x => x.Key == 263.5M).OrderByDescending(x => x.Key).SingleOrDefault();
			Assert.That(result.Key, Is.EqualTo(263.5M));
			Assert.That(result.Count, Is.EqualTo(1));
		}
		...
		{
			var result = db.Products.GroupBy(x => x.Supplier.CompanyName).Select(x => new
			{
		...
		{
			var result = db.Products.GroupBy(x => x.Supplier.CompanyName).Where(x => x.Count(y => y.UnitPrice == 12.75M) == 1).Select(x => new
			{
			x.Key, Count = x.Count()}
		...
		public void FilteredByCountFromSubQuery()
		{
			...
			var result = db.Products.GroupBy(x => x.Supplier.CompanyName).Where(x => db.Products.Count(y => y.Supplier.CompanyName == x.Key && y.UnitPrice == 12.75M) == 1).Select(x => new
			{
			x.Key, Count = x.Count()}
		...
		{
			var result = db.Products.GroupBy(x => x.Supplier.CompanyName).Where(x => x.Sum(y => y.UnitPrice == 12.75M ? y.UnitPrice : 0M) == 12.75M).Select(x => new
			{
		...
		{
			var result = db.Products.GroupBy(x => x.Supplier.CompanyName).Where(x => x.Key == "Zaanse Snoepfabriek").Select(x => new
			{
		...
		{
			var result = db.Products.GroupBy(x => x.Supplier.Address.Country).OrderBy(x => x.Key).Select(x => new
			{
		...
		{
			var orderGroups = db.Orders.GroupBy(o => o.Customer.CustomerId == null ? 0 : 1).Select(g => new
			{
		...
		{
			var orderGroups = db.Orders.GroupBy(o => new
			{
		...
		{
			var orderGroups = db.Orders.GroupBy(o => new[]{o.Customer.CustomerId == null ? 0 : 1, }).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var totals = db.Orders.GroupBy(o => 1).Select(g => new
			{
		...
		{
			var totals = db.Orders.GroupBy(o => new
			{
			A = 1
			}
		...
		{
			var totals = db.Orders.GroupBy(o => new object[]{1}).Select(g => new
			{
			Key = g.Key, Count = g.Count(), Sum = g.Sum(x => x.Freight)}
		...
		{
			var totals = db.Orders.GroupBy(o => new[]{1, o.Shipper.ShipperId}).Select(g => new
			{
			Key = g.Key, Count = g.Count(), Sum = g.Sum(x => x.Freight)}
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => o.Order.Customer == null ? 0 : 1).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => o.Order.Customer.CustomerId == null ? 0 : 1).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => new
			{
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => new[]{o.Order.Customer == null ? 0 : 1}).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => new[]{o.Order.Customer.CustomerId == null ? 0 : 1}).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var orderGroups = db.OrderLines.GroupBy(o => new[]{o.Order.Customer.CustomerId == null ? "unknown" : o.Order.Customer.CompanyName}).Select(g => new
			{
			Key = g.Key, Count = g.Count()}
		...
		{
			var result = db.Orders.Fetch(x => x.Customer).GroupBy(x => x.Customer.CompanyName).OrderBy(x => x.Key).Select(x => new
			{
			P0 = x.Key, P1 = x.Count()}
