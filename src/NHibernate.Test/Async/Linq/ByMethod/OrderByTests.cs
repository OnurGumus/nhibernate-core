...
namespace NHibernate.Test.Linq.ByMethod
{
	...
	public partial class OrderByTestsAsync : LinqTestCaseAsync
	{
		...
		public void OrderByCalculatedAggregatedSubselectProperty()
		{
			...
			{
			o.OrderId, TotalQuantity = o.OrderLines.Sum(c => c.Quantity)}
		...
		public void OrderByWithSelfReferencedSubquery1()
		{
			...
			var result = (
				...
				where order == db.Orders.OrderByDescending(x => x.OrderDate).First(x => x.Customer == order.Customer)orderby order.Customer.CustomerId
				select order).ToList();
		...
		public void OrderByWithSelfReferencedSubquery2()
		{
			...
			var result = (
				from order in db.Orders
				where order == db.Orders.OrderByDescending(x => x.OrderDate).First(x => x.Customer == order.Customer)orderby order.ShippingDate descending
				select order).ToList();
