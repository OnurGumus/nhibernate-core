...
namespace NHibernate.Test.Linq.ByMethod
{
	...
	public partial class GetValueOrDefaultTestsAsync : LinqTestCaseAsync
	{
		...
		{
			var orders = db.Orders.Where(x => (x.Freight ?? 100) > 0).ToList();
		...
		{
			var orders = db.Orders.Where(x => x.Freight.GetValueOrDefault(100) > 0).ToList();
			Assert.AreEqual(830, orders.Count);
		}
		...
		{
			var orders = db.Orders.Where(x => x.Freight.GetValueOrDefault() > 0).ToList();
			Assert.AreEqual(830, orders.Count);
		}
	}
}
