...
namespace NHibernate.Test.Linq
{
	...
	public partial class WhereSubqueryTestsAsync : LinqTestCaseAsync
	{
		...
		public void TimeSheetsWithCountSubqueryReversed()
		{
			...
			var query = (
				from timesheet in db.Timesheets
				where 1 <= timesheet.Entries.Count()select timesheet).ToList();
			// ReSharper restore UseMethodAny.1
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithCountSubqueryComparedToPropertyReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Count()select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithAverageSubqueryReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where 12 < timesheet.Entries.Average(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithAverageSubqueryComparedToPropertyReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Id > timesheet.Entries.Average(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithMaxSubqueryReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where 14 == timesheet.Entries.Max(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithMaxSubqueryComparedToPropertyReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Max(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithMinSubqueryReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where 7 > timesheet.Entries.Min(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithMinSubqueryComparedToPropertyReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Min(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithSumSubqueryReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where 20 >= timesheet.Entries.Sum(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithSumSubqueryComparedToPropertyReversed()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Id >= timesheet.Entries.Sum(e => e.NumberOfHours)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void TimeSheetsWithStringContainsSubQuery()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Entries.Any(e => e.Comments.Contains("testing"))select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithStringContainsSubQueryWithAsQueryable()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(e => e.Comments.Contains("testing"))select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithStringContainsSubQueryWithAsQueryableAndExternalPredicate()
		{
			...
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(predicate)select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void CategoriesSubQueryWithAsQueryableAndExternalPredicateWithClosure()
		{
			...
			var query = (
				...
				where category.Products.AsQueryable().Any(predicate1)select category).ToList();
		...
		public void TimeSheetsSubQueryWithAsQueryableWithArray()
		{
			var query = (
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(e => !new[]{1}.Contains(e.Id))select timesheet).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
...
").ListAsync<OrderLine>());
		...
		public void OrderLinesWithImpliedJoinAndSubQuery()
		{
			...
			var lines = (
				...
				where !l.Order.Employee.Subordinates.Any(x => x.EmployeeId == 100)select l).ToList();
		...
		public void OrdersWithSubquery1()
		{
			var query = (
				...
				where order.OrderLines.Any()select order).ToList();
		...
		public void OrdersWithSubquery2()
		{
			...
			var query = (
				from order in db.Orders
				where subquery.Contains(order)select order).ToList();
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public void OrdersWithSubquery3()
		{
			...
			var query = (
				from order in db.Orders
				where subquery.Contains(order.OrderId)select order).ToList();
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public void OrdersWithSubquery4()
		{
			...
			var query = (
				from order in db.Orders
				where subquery.Any(x => x.OrderId == order.OrderId)select order).ToList();
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public void OrdersWithSubquery5()
		{
			var query = (
				from order in db.Orders
				where order.OrderLines.Any(x => x.Quantity == 5)select order).ToList();
		...
		public void CategoriesWithDiscountedProducts()
		{
			var query = (
				...
				where c.Products.Any(p => p.Discontinued)select c).ToList();
		...
		public void ProductsWithSubquery()
		{
			var result = (
				...
				where (
					...
					select c).Contains(p.Category)select p).ToList();
		...
		public void ProductsWithSubqueryAsIEnumerable()
		{
			...
			var result = (
				from p in db.Products
				where categories.Contains(p.Category)select p).ToList();
			Assert.That(result.Count, Is.EqualTo(13));
		}
		...
		public void SubqueryWithGroupBy()
		{
			...
			var result = db.Orders.Where(x => sq.Contains(x.OrderId)).Select(x => x.OrderId).ToList();
		...
		{
			var list = (db.OrderLines.Select(ol => new
			{
		...
		public void NullComparedToNewExpressionInWhereClause()
		{
			...
			).Where(pr1 => (pr1.Pr2 == null ? (int ? )null : pr1.Pr2.ReorderLevel) > 6).ToList();
		...
		public void NullComparedToMemberInitExpressionInWhereClause()
		{
			// Construction will never be equal to null, so the ternary should be collapsed
			// to just the IfFalse expression. Without this collapsing, we cannot generate HQL.
			var result = db.Products.Select(p => new Pr1{Name = p.Name, Pr2 = new Pr2{ReorderLevel = p.ReorderLevel}}).Where(pr1 => (pr1.Pr2 == null ? (int ? )null : pr1.Pr2.ReorderLevel) > 6).ToList();
			Assert.That(result.Count, Is.EqualTo(45));
		}
	}
}
