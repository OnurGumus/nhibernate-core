...
namespace NHibernate.Test.Linq
{
	...
	public partial class WhereSubqueryTestsAsync : LinqTestCaseAsync
	{
		...
		public async Task TimeSheetsWithCountSubqueryReversedAsync()
		{
			...
			var query = await ((
				from timesheet in db.Timesheets
				where 1 <= timesheet.Entries.Count()select timesheet).ToListAsync());
			// ReSharper restore UseMethodAny.1
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithCountSubqueryComparedToPropertyReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Count()select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithAverageSubqueryReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where 12 < timesheet.Entries.Average(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithAverageSubqueryComparedToPropertyReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Id > timesheet.Entries.Average(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithMaxSubqueryReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where 14 == timesheet.Entries.Max(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithMaxSubqueryComparedToPropertyReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Max(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithMinSubqueryReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where 7 > timesheet.Entries.Min(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithMinSubqueryComparedToPropertyReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Id < timesheet.Entries.Min(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithSumSubqueryReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where 20 >= timesheet.Entries.Sum(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithSumSubqueryComparedToPropertyReversedAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Id >= timesheet.Entries.Sum(e => e.NumberOfHours)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public async Task TimeSheetsWithStringContainsSubQueryAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Entries.Any(e => e.Comments.Contains("testing"))select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithStringContainsSubQueryWithAsQueryableAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(e => e.Comments.Contains("testing"))select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task TimeSheetsWithStringContainsSubQueryWithAsQueryableAndExternalPredicateAsync()
		{
			...
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(predicate)select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public async Task CategoriesSubQueryWithAsQueryableAndExternalPredicateWithClosureAsync()
		{
			...
			Expression<Func<Product, bool>> predicate1 = e => !ids.Contains(e.ProductId) && e.OrderLines.AsQueryable().Any(predicate2);
			var query = await ((
				from category in db.Categories
				where category.Products.AsQueryable().Any(predicate1)select category).ToListAsync());
		...
		public async Task TimeSheetsSubQueryWithAsQueryableWithArrayAsync()
		{
			var query = await ((
				from timesheet in db.Timesheets
				where timesheet.Entries.AsQueryable().Any(e => !new[]{1}.Contains(e.Id))select timesheet).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(2));
		}
...
").ListAsync<OrderLine>());
		...
		public async Task OrderLinesWithImpliedJoinAndSubQueryAsync()
		{
			...
			var lines = await ((
				...
				where !l.Order.Employee.Subordinates.Any(x => x.EmployeeId == 100)select l).ToListAsync());
		...
		public async Task OrdersWithSubquery1Async()
		{
			var query = await ((
				...
				where order.OrderLines.Any()select order).ToListAsync());
		...
		public async Task OrdersWithSubquery2Async()
		{
			...
			var query = await ((
				from order in db.Orders
				where subquery.Contains(order)select order).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public async Task OrdersWithSubquery3Async()
		{
			...
			var query = await ((
				from order in db.Orders
				where subquery.Contains(order.OrderId)select order).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public async Task OrdersWithSubquery4Async()
		{
			...
			var query = await ((
				from order in db.Orders
				where subquery.Any(x => x.OrderId == order.OrderId)select order).ToListAsync());
			Assert.That(query.Count, Is.EqualTo(830));
		}
		...
		public async Task OrdersWithSubquery5Async()
		{
			var query = await ((
				from order in db.Orders
				where order.OrderLines.Any(x => x.Quantity == 5)select order).ToListAsync());
		...
		public async Task CategoriesWithDiscountedProductsAsync()
		{
			var query = await ((
				...
				where c.Products.Any(p => p.Discontinued)select c).ToListAsync());
		...
		public async Task OrdersWithSubqueryWithJoinAsync()
		{
			var subquery =
				from line in db.OrderLines
				join product in db.Products on line.Product.ProductId equals product.ProductId
				where line.Quantity == 5
				select line.Order;
			var query = await ((
				from order in db.Orders
		...
		public async Task ProductsWithSubqueryAsync()
		{
			var result = await ((
				...
				where (
					...
					select c).Contains(p.Category)select p).ToListAsync());
		...
		public async Task ProductsWithSubqueryAsIEnumerableAsync()
		{
			...
			var result = await ((
				from p in db.Products
				where categories.Contains(p.Category)select p).ToListAsync());
			Assert.That(result.Count, Is.EqualTo(13));
		}
		...
		{
			var sq = db.Orders.GroupBy(x => x.ShippingDate).Select(x => x.Max(o => o.OrderId));
			var result = await (db.Orders.Where(x => sq.Contains(x.OrderId)).Select(x => x.OrderId).ToListAsync());
		...
		{
			var list = await ((db.OrderLines.Select(ol => new
			{
			ol.Discount, ShipperPhoneNumber = db.Shippers.Where(sh => sh.ShipperId == ol.Order.Shipper.ShipperId).Select(sh => sh.PhoneNumber).FirstOrDefault()}
		...
		public async Task SubqueryWhereFailingTest2Async()
		{
			var list = await (db.OrderLines.Select(ol => new
			{
			ol.Discount, ShipperPhoneNumber = db.Shippers.Where(sh => sh == ol.Order.Shipper).Select(sh => sh.PhoneNumber).FirstOrDefault()}
		...
		public async Task SubqueryWhereFailingTest3Async()
		{
			var list = await (db.OrderLines.Select(ol => new
			{
			ol.Discount, ShipperPhoneNumber = db.Orders.Where(sh => sh.Shipper.ShipperId == ol.Order.Shipper.ShipperId).Select(sh => sh.Shipper.PhoneNumber).FirstOrDefault()}
		...
		public async Task ProductsWithSubqueryReturningBoolFirstOrDefaultEqAsync()
		{
			var result = await ((
				...
				where (
					from c in db.Categories
					where c.Name == "Confections" && c == p.Category
		...
		public async Task NullComparedToNewExpressionInWhereClauseAsync()
		{
			...
			var result = await (db.Products.Select(p => new
			{
			Name = p.Name, Pr2 = new
			{
			...
			).Where(pr1 => (pr1.Pr2 == null ? (int ? )null : pr1.Pr2.ReorderLevel) > 6).ToListAsync());
		...
		public async Task NullComparedToMemberInitExpressionInWhereClauseAsync()
		{
			// Construction will never be equal to null, so the ternary should be collapsed
			// to just the IfFalse expression. Without this collapsing, we cannot generate HQL.
			var result = await (db.Products.Select(p => new Pr1{Name = p.Name, Pr2 = new Pr2{ReorderLevel = p.ReorderLevel}}).Where(pr1 => (pr1.Pr2 == null ? (int ? )null : pr1.Pr2.ReorderLevel) > 6).ToListAsync());
			Assert.That(result.Count, Is.EqualTo(45));
		}
	}
}
