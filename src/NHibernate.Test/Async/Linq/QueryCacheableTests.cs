...
namespace NHibernate.Test.Linq
{
	...
	public partial class QueryCacheableTestsAsync : LinqTestCaseAsync
	{
		...
		public void QueryIsCacheable3()
		{
			...
			Sfi.QueryCache.Clear();
			var x = (
				from c in db.Customers.Cacheable()select c).ToList();
			var x2 = (
				from c in db.Customers
				select c).ToList();
			Assert.That(Sfi.Statistics.QueryExecutionCount, Is.EqualTo(2));
		...
		public void GroupByQueryIsCacheable()
		{
			Sfi.Statistics.Clear();
			Sfi.QueryCache.Clear();
			var c = db.Customers.GroupBy(x => x.Address.Country).Select(x => x.Key).Cacheable().ToList();
			c = db.Customers.GroupBy(x => x.Address.Country).Select(x => x.Key).ToList();
			c = db.Customers.GroupBy(x => x.Address.Country).Select(x => x.Key).Cacheable().ToList();
			Assert.That(Sfi.Statistics.QueryExecutionCount, Is.EqualTo(2));
			Assert.That(Sfi.Statistics.QueryCachePutCount, Is.EqualTo(1));
			Assert.That(Sfi.Statistics.QueryCacheHitCount, Is.EqualTo(1));
		}
		...
		public void GroupByQueryIsCacheable2()
		{
			Sfi.Statistics.Clear();
			Sfi.QueryCache.Clear();
			var c = db.Customers.Cacheable().GroupBy(x => x.Address.Country).Select(x => x.Key).ToList();
			c = db.Customers.GroupBy(x => x.Address.Country).Select(x => x.Key).ToList();
			c = db.Customers.Cacheable().GroupBy(x => x.Address.Country).Select(x => x.Key).ToList();
			Assert.That(Sfi.Statistics.QueryExecutionCount, Is.EqualTo(2));
			Assert.That(Sfi.Statistics.QueryCachePutCount, Is.EqualTo(1));
			Assert.That(Sfi.Statistics.QueryCacheHitCount, Is.EqualTo(1));
		}
