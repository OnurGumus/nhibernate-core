...
namespace NHibernate.Test.Linq
{
	...
	public partial class MethodCallTestsAsync : LinqTestCaseAsync
	{
		...
		{
			var result = db.Users.Select(u => new object[]{u.Id, u.Name, u.InvalidLoginAttempts}).First();
		...
		{
			var result = db.Users.Select(u => new object[]{u.Component, u.Component.OtherComponent}).First();
		...
		{
			var result = db.Users.Where(u => u.Name == "nhibernate").Select(u => new object[]{u.Enum1, u.Enum2, u.Features}).First();
			Assert.That(result.Length, Is.EqualTo(3));
		...
		public void CanSelectConstantsIntoObjectArray()
		{
			...
			var result = db.Users.Select(u => new object[]{u.Id, pi, name, DateTime.MinValue}).First();
		...
		{
			var result = db.Users.Select(u => new object[]{u.Id, u.Role.Name, u.Role.Entity.Output}).First();
			Assert.That(result.Length, Is.EqualTo(3));
