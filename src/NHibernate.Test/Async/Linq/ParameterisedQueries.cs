...
namespace NHibernate.Test.Linq
{
	...
	public partial class ParameterisedQueriesAsync : ReadonlyTestCaseAsync
	{
		[Test]
		...
		public void CanSpecifyParameterTypeInfo()
		{
			using (var s = OpenSession())
			{
				...
				Expression<Func<IEnumerable<Customer>>> london = () =>
					from c in db.Customers
					where c.Address.City == "London".MappedAs(NHibernateUtil.StringClob)select c;
				Expression<Func<IEnumerable<Customer>>> newYork = () =>
					from c in db.Customers
					where c.Address.City == "New York".MappedAs(NHibernateUtil.AnsiString)select c;
				var nhLondon = new NhLinqExpression(london.Body, sessions);
				var nhNewYork = new NhLinqExpression(newYork.Body, sessions);
		...
		public void Different_Select_Member_Initialisation_Returns_Different_Keys()
		{
			using (var s = OpenSession())
			{
				...
				Expression<Func<IEnumerable>> newCustomerId = () =>
					...
					select new
					{
					Id = c.CustomerId, Title = c.ContactTitle
					}
				...
				Expression<Func<IEnumerable>> customerId = () =>
					...
					select new
					{
					Title = c.ContactTitle, Id = c.CustomerId
					}
		...
		public void Different_Conditionals_Return_Different_Keys()
		{
			using (var s = OpenSession())
			{
				...
				Expression<Func<IEnumerable>> newCustomerId = () =>
					...
					select new
					{
					Desc = c.CustomerId == "1" ? "First" : "Not First"
					}
		...
		public void Different_OfType_Returns_Different_Keys()
		{
			...
			{
				Expression<Func<IEnumerable>> ofType1 = () => (
					from a in session.Query<Animal>().OfType<Cat>()where a.Pregnant
					select a.Id);
				Expression<Func<IEnumerable>> ofType2 = () => (
					from a in session.Query<Animal>().OfType<Dog>()where a.Pregnant
					select a.Id);
		...
		public void Different_Null_Returns_Different_Keys()
		{
			using (var session = OpenSession())
			{
				...
				Expression<Func<IEnumerable>> null1 = () => (
					from a in session.Query<Animal>()where a.Description == null
					select a);
				Expression<Func<IEnumerable>> null2 = () => (
					from a in session.Query<Animal>()where a.Description == nullVariable
					select a);
				Expression<Func<IEnumerable>> notNull = () => (
					from a in session.Query<Animal>()where a.Description == notNullVariable
					select a);
