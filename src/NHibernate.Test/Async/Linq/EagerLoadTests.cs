...
namespace NHibernate.Test.Linq
{
	...
	public partial class EagerLoadTestsAsync : LinqTestCaseAsync
	{
		...
		{
			//NH-3075
			var result = db.Orders.Select(o => o.Customer).Fetch(c => c.Orders).ToList();
			session.Close();
		...
		{
			//NH-2915
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
		...
		public void WhereBeforeFetchAndSelect()
		{
			//NH-3056
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).Fetch(x => x.Customer).Select(x => x).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].Customer));
		}
		...
		public void WhereBeforeFetchManyAndSelect()
		{
			//NH-3056
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).FetchMany(x => x.OrderLines).Select(x => x).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines));
		}
		...
		public void WhereBeforeFetchManyThenFetchAndSelect()
		{
			//NH-3056
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).FetchMany(x => x.OrderLines).ThenFetch(x => x.Product).Select(x => x).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines));
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines.First().Product));
		}
		...
		{
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).Fetch(x => x.Customer).Where(x => true).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].Customer));
		}
		...
		{
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).FetchMany(x => x.OrderLines).Where(x => true).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines));
		}
		...
		{
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var orders = db.Orders.Where(x => x.OrderId != firstOrderId).FetchMany(x => x.OrderLines).ThenFetch(x => x.Product).Where(x => true).ToList();
			Assert.AreEqual(829, orders.Count);
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines));
			Assert.IsTrue(NHibernateUtil.IsInitialized(orders[0].OrderLines.First().Product));
		}
		...
		public void WhereAfterFetchAndSingleOrDefault()
		{
			//NH-3186
			var firstOrderId = db.Orders.OrderBy(x => x.OrderId).Select(x => x.OrderId).First();
			var order = db.Orders.Fetch(x => x.Shipper).SingleOrDefault(x => x.OrderId == firstOrderId);
