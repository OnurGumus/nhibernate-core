	...
	public partial class BinaryExpressionOrdererTestsAsync : LinqTestCaseAsync
	{
		...
		public void ValuePropertySwapsToPropertyValue()
		{
			var query = (
				...
				where ("ayende" == user.Name)select user).ToList();
		...
		public void PropertyValueDoesntSwaps()
		{
			var query = (
				from user in db.Users
				where (user.Name == "ayende")select user).ToList();
			Assert.AreEqual(1, query.Count);
		}
		...
		public void PropertyPropertyDoesntSwap()
		{
			var query = (
				from user in db.Users
				where (user.Name == user.Name)select user).ToList();
		...
		public void GreaterThanSwapsToLessThan()
		{
			var query = (
				from user in db.Users
				where (3 > user.Id)select user).ToList();
			Assert.AreEqual(2, query.Count);
		}
		...
		public void GreaterThanOrEqualToSwapsToLessThanOrEqualTo()
		{
			var query = (
				from user in db.Users
				where (2 >= user.Id)select user).ToList();
			Assert.AreEqual(2, query.Count);
		}
		...
		public void LessThanSwapsToGreaterThan()
		{
			var query = (
				from user in db.Users
				where (1 < user.Id)select user).ToList();
			Assert.AreEqual(2, query.Count);
		}
		...
		public void ValuePropertySwapsToPropertyValueUsingEqualsFromConstant()
		{
			...
			var query = (
				from user in db.Users
				where ("ayende".Equals(user.Name))select user).ToList();
			Assert.AreEqual(1, query.Count);
		}
		...
		public void ValuePropertySwapsToPropertyValueUsingEqualsToConstant()
		{
			...
			var query = (
				from user in db.Users
				where (user.Name.Equals("ayende"))select user).ToList();
			Assert.AreEqual(1, query.Count);
		}
	}
}
