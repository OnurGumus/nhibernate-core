...
namespace NHibernate.Test.Linq
{
	...
	public partial class WhereTestsAsync : LinqTestCaseAsync
	{
		...
		[Test]
		public Task FirstElementWithQueryThatReturnsNoResultsAsync()
		{
			try
			{
				var users =
		...
		[Test]
		public Task SingleElementWithQueryThatReturnsNoResultsAsync()
		{
			try
			{
				var users =
		...
		[Test]
		public Task SingleElementWithQueryThatReturnsMultipleResultsAsync()
		{
			try
			{
				var users =
		...
		public void UsersRegisteredAtOrAfterY2K()
		{
			var query = (
				from user in db.Users
				where user.RegisteredAt >= new DateTime(2000, 1, 1)select user).ToList();
		...
		public void UsersRegisteredAtOrAfterY2K_And_Before2001()
		{
			var query = (
				from user in db.Users
				where user.RegisteredAt >= new DateTime(2000, 1, 1) && user.RegisteredAt <= new DateTime(2001, 1, 1)select user).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void UsersByNameAndRegistrationDate()
		{
			var query = (
				from user in db.Users
				where user.Name == "ayende" && user.RegisteredAt == new DateTime(2010, 06, 17)select user).FirstOrDefault();
		...
		public void UsersWithStringContains()
		{
			var query = (
				from user in db.Users
				where user.Name.Contains("yend")select user).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void UsersWithStringContainsAndNotNullName()
		{
			...
			var query = (
				from u in db.Users
				where u.Name == null ? false : u.Name.Contains("yend")select u).ToList();
			// ReSharper restore SimplifyConditionalTernaryExpression
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void UsersWithArrayContains()
		{
			...
			var query = (
				from user in db.Users
				where names.Contains(user.Name)select user).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void UsersWithEmptyInlineEnumerable()
		{
			...
			var query = (
				from user in db.Users
				where allNames.Where(n => n == "does not exist").Contains(user.Name)select user).ToList();
			Assert.That(query.Count, Is.EqualTo(0));
		}

		[Test]
		public Task WhenTheSourceOfConstantIsICollectionThenNoThrowsAsync()
		{
			try
			{
		...
		[Test]
		public Task WhenTheSourceOfConstantIsIListThenNoThrowsAsync()
		{
			try
			{
		...
		{
			var query = (
				from o in session.Query<Animal>()select o).OfType<Dog>().ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		{
			var query = (
				from o in session.Query<Dog>()select o).Cast<Animal>().ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		{
			var query = (
				from Dog o in session.Query<Dog>()select o).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void TimeSheetsWithSamePredicateTwoTimes()
		{
			...
			var query = db.Timesheets.Where(predicate).Where(predicate).ToList();
			Assert.That(query.Count, Is.EqualTo(2));
		}
		...
		public void AnimalsWithFathersSerialNumberListContains()
		{
			...
			var query = (
				...
				where animal.Father != null && serialNumbers.Contains(animal.Father.SerialNumber)select animal).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
		...
		public void AnimalsWithFathersSerialNumberListContainsWithLocalVariable()
		{
			...
			var query = (
				...
				where father != null && serialNumbers.Contains(father.SerialNumber)select animal).ToList();
			Assert.That(query.Count, Is.EqualTo(1));
		}
