...
namespace NHibernate.Test.Linq
{
	...
	public partial class PagingTestsAsync : LinqTestCaseAsync
	{
		...
		{
			// NH-3326
			var list = db.Products.Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).Skip(5).Take(10).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}
		...
		{
			// Variation of NH-3326 with order
			var list = db.Products.Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).OrderBy(x => x.ProductId).Skip(5).Take(10).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}
		...
		{
			// Variation of NH-3326 with where
			var list = db.Products.Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).Where(p => p.ProductId > 0).OrderBy(x => x.ProductId).Skip(5).Take(10).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}
		...
		{
			// NH-2588 and NH-3326
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Select(p => new
			{
			...
			var ids = db.Products.OrderByDescending(x => x.ProductId).Select(p => new
			{
			p.ProductId, p.Name, p.UnitsInStock
			}
		...
		public void PagedProductsWithOuterWhereClauseOrderedNestedAnonymousEquivalent()
		{
			...
			var ids = db.Products.Select(p => new
			{
			p.ProductId, p.Name, p.UnitsInStock
			}
		...
		{
			// NH-2588 and NH-3326
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).Skip(10).Take(20).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}

			).Where(x => x.ProductId > 0).ToList();
			var ids = db.Products.OrderByDescending(x => x.ProductId).Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).Skip(10).Take(20).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}
		...
		public void PagedProductsWithOuterWhereClauseOrderedNestedClassEquivalent()
		{
			...
			var ids = db.Products.Select(p => new ProductProjection{ProductId = p.ProductId, Name = p.Name}).Where(x => subquery.Contains(x)).Where(x => x.ProductId > 0).Select(a => new
			{
			ExpandedElement = a, a.Name, a.ProductId
			}
		...
		{
			//NH-3108
			var inMemoryIds = db.Products.ToList().OrderBy(p => p.ProductId).Skip(10).Take(20).Select(p => p.ProductId).ToList();
			var ids = db.Products.OrderBy(p => p.ProductId).Skip(10).Take(20).Select(p => p.ProductId).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		{
			//NH-3108 (not failing)
			var inMemoryIds = db.Products.ToList().OrderBy(p => p.ProductId).Select(p => p.ProductId).Skip(10).Take(20).ToList();
			var ids = db.Products.OrderBy(p => p.ProductId).Select(p => p.ProductId).Skip(10).Take(20).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		{
			//NH-3108
			var inMemoryIds = db.Products.ToList().OrderByDescending(p => p.ProductId).Skip(10).Take(20).Select(p => p.ProductId).ToList();
			var ids = db.Products.OrderByDescending(p => p.ProductId).Skip(10).Take(20).Select(p => p.ProductId).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		{
			//NH-3108 (not failing)
			var inMemoryIds = db.Products.ToList().OrderByDescending(p => p.ProductId).Select(p => p.ProductId).Skip(10).Take(20).ToList();
			var ids = db.Products.OrderByDescending(p => p.ProductId).Select(p => p.ProductId).Skip(10).Take(20).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		public void PagedProductsWithOuterWhereClause()
		{
			...
			//NH-2588
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).ToList();
			var ids = db.Products.OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		public void PagedProductsWithOuterWhereClauseResort()
		{
			if (Dialect is MySQLDialect)
				Assert.Ignore("MySQL does not support LIMIT in subqueries.");
			//NH-2588
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).OrderBy(x => x.Name).ToList();
		...
		public void PagedProductsWithInnerAndOuterWhereClauses()
		{
			if (Dialect is MySQLDialect)
				Assert.Ignore("MySQL does not support LIMIT in subqueries.");
			//NH-2588
			var inMemoryIds = db.Products.ToList().Where(x => x.UnitsInStock < 100).OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).OrderBy(x => x.Name).ToList();
		...
		public void PagedProductsWithOuterWhereClauseAndProjection()
		{
			if (Dialect is MySQLDialect)
				Assert.Ignore("MySQL does not support LIMIT in subqueries.");
			//NH-2588
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).Select(x => x.ProductId).ToList();
			var ids = db.Products.OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).Select(x => x.ProductId).ToList();
			Assert.That(ids, Is.EqualTo(inMemoryIds));
		}
		...
		public void PagedProductsWithOuterWhereClauseAndComplexProjection()
		{
			if (Dialect is MySQLDialect)
				Assert.Ignore("MySQL does not support LIMIT in subqueries.");
			//NH-2588
			var inMemoryIds = db.Products.ToList().OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).Select(x => new
			{
			x.ProductId
			}

			).ToList();
			var ids = db.Products.OrderByDescending(x => x.ProductId).Skip(10).Take(20).Where(x => x.UnitsInStock > 0).Select(x => new
			{
			x.ProductId
			}
