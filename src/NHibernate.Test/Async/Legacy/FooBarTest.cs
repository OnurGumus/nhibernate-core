	...
	public class FooBarTestAsync : TestCase
	{
		...
		public async Task CollectionVersioningAsync()
		{
			using (ISession s = OpenSession())
			{
				...
				await (s.DeleteAsync(many, CancellationToken.None));
		...
		public async Task ForCertainAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				{
					gid = await (s.SaveAsync(g, CancellationToken.None));
					g2id = await (s.SaveAsync(g2, CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					g = (Glarch) await (s.GetAsync(typeof(Glarch), gid, CancellationToken.None));
					g2 = (Glarch) await (s.GetAsync(typeof(Glarch), g2id, CancellationToken.None));
					Assert.AreEqual(1, g2.Strings.Count);

					await (s.DeleteAsync(g, CancellationToken.None));
					await (s.DeleteAsync(g2, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
		}
		...
		public async Task BagMultipleElementsAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				{
					//put in cache
					Baz baz = (Baz) await (s.GetAsync(typeof(Baz), bazCode, CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					...
					await (s.DeleteAsync(baz, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
		}
		...
		public async Task WierdSessionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					id = await (s.SaveAsync(new Foo(), CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
			...
			using (ISession s = OpenSession())
			{
				...
				using (ITransaction t = s.BeginTransaction())
				{
					Foo foo = (Foo) await (s.GetAsync(typeof(Foo), id, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			...
			using (ISession s = OpenSession())
			{
				...
				{
					Foo foo = (Foo) await (s.GetAsync(typeof(Foo), id, CancellationToken.None));
					await (s.DeleteAsync(foo, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
		}
		...
		public async Task DereferenceLazyCollectionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				Foo foo = (Foo) await (s.GetAsync(typeof(Foo), fooKey, CancellationToken.None));
		...
		public async Task MoveLazyCollectionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(foo, CancellationToken.None));
				await (s.SaveAsync(baz, CancellationToken.None));
				await (s.SaveAsync(baz2, CancellationToken.None));
				foo.Bytes = GetBytes("foobar");
				await (s.FlushAsync(CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				Baz baz = (Baz) await (s.GetAsync(typeof(Baz), bazCode, CancellationToken.None));
				Assert.IsFalse(NHibernateUtil.IsInitialized(baz.FooSet));
				Baz baz2 = (Baz) await (s.GetAsync(typeof(Baz), baz2Code, CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.DeleteAsync(baz, CancellationToken.None));
				await (s.DeleteAsync(baz2, CancellationToken.None));
				await (s.DeleteAsync(foo, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task CriteriaCollectionAsync()
		{
			...
			Baz bb = (Baz) await (s.CreateCriteria(typeof(Baz)).UniqueResultAsync(CancellationToken.None));
			Baz baz = new Baz();
			await (s.SaveAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			Baz b = (Baz) await (s.CreateCriteria(typeof(Baz)).UniqueResultAsync(CancellationToken.None));
		...
		public async Task QueryAsync()
		{
			...
			await (s.SaveAsync(foo, CancellationToken.None));
			Foo foo2 = new Foo();
			await (s.SaveAsync(foo2, CancellationToken.None));
			foo.TheFoo = foo2;

			IList list = await (s.CreateQuery("from Foo foo inner join fetch foo.TheFoo").ListAsync(CancellationToken.None));
			Foo foof = (Foo) list[0];
			Assert.IsTrue(NHibernateUtil.IsInitialized(foof.TheFoo));

			list = await (s.CreateQuery("from Baz baz left outer join fetch baz.FooToGlarch").ListAsync(CancellationToken.None));

			list = await (s.CreateQuery("select foo, bar from Foo foo left outer join foo.TheFoo bar where foo = ?")
				.SetEntity(0, foo).ListAsync(CancellationToken.None));


			object[] row1 = (object[]) list[0];
			Assert.IsTrue(row1[0] == foo && row1[1] == foo2);

			await (s.CreateQuery("select foo.TheFoo.TheFoo.String from foo in class Foo where foo.TheFoo = 'bar'").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.TheFoo.TheFoo.TheFoo.String from foo in class Foo where foo.TheFoo.TheFoo = 'bar'").ListAsync(CancellationToken.None));
			...
			//			{
			await (s.CreateQuery("select foo.String from foo in class Foo where foo.TheFoo.TheFoo.TheFoo = foo.TheFoo.TheFoo").ListAsync(CancellationToken.None));
			//			}
			await (s.CreateQuery(
			...
			await (s.CreateQuery("from bar in class Bar, foo in elements(bar.Baz.FooArray)").ListAsync(CancellationToken.None));

			if (Dialect is DB2Dialect)
			{
				await (s.CreateQuery("from foo in class Foo where lower( foo.TheFoo.String ) = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from foo in class Foo where lower( (foo.TheFoo.String || 'foo') || 'bar' ) = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from foo in class Foo where repeat( (foo.TheFoo.STring || 'foo') || 'bar', 2 ) = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery(
			...
			{
				await (s.CreateQuery("select baz from Baz as baz join baz.FooArray foo group by baz order by sum(foo.Float)").EnumerableAsync(CancellationToken.None));
			}

			await (s.CreateQuery("from Foo as foo where foo.Component.Glarch.Name is not null").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Foo as foo left outer join foo.Component.Glarch as glarch where glarch.Name = 'foo'").ListAsync(CancellationToken.None));

			list = await (s.CreateQuery("from Foo").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count);
			Assert.IsTrue(list[0] is FooProxy);
			list = await (s.CreateQuery("from Foo foo left outer join foo.TheFoo").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count);
			Assert.IsTrue(((object[]) list[0])[0] is FooProxy);

			await (s.CreateQuery("From Foo, Bar").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Baz baz left join baz.FooToGlarch, Bar bar join bar.TheFoo").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Baz baz left join baz.FooToGlarch join baz.FooSet").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Baz baz left join baz.FooToGlarch join fetch baz.FooSet foo left join fetch foo.TheFoo").ListAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery("select foo.TheFoo from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None));
			...
			if (Dialect.SupportsSubSelects && TestDialect.SupportsOperatorSome)
			{
				if (!(Dialect is FirebirdDialect))
				{
					list = await (s.CreateQuery(
							...
							SetDateTime(0, DateTime.Today).ListAsync(CancellationToken.None));
					
					Assert.AreEqual(2, list.Count, "component query");
				}

				list =
					await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where size(foo.Component.ImportantDates) = 3").ListAsync(CancellationToken.None));
				Assert.AreEqual(2, list.Count, "component query");
				list = await (s.CreateQuery("from foo in class Foo where 0 = size(foo.Component.ImportantDates)").ListAsync(CancellationToken.None));
				Assert.AreEqual(0, list.Count, "component query");
				list = await (s.CreateQuery("from foo in class Foo where exists elements(foo.Component.ImportantDates)").ListAsync(CancellationToken.None));
				Assert.AreEqual(2, list.Count, "component query");
				await (s.CreateQuery("from foo in class Foo where not exists (from bar in class Bar where bar.id = foo.id)").ListAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery("from foo in class Foo where foo.Component.Count is null order by foo.Component.Count").ListAsync(CancellationToken.None));
			Assert.AreEqual(0, list.Count, "component query");

			list = await (s.CreateQuery("from foo in class Foo where foo.Component.Name='foo'").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count, "component query");
			...
			list = await (s.CreateQuery("select foo.TheFoo from foo in class Foo").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count, "query");

			list = await (s.CreateQuery("from foo in class Foo where foo.id=?").SetString(0, foo.Key).ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "id query");

			list = await (s.CreateQuery("from foo in class Foo where foo.Key=?").SetString(0, foo.Key).ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "named id query");
			Assert.AreSame(foo, list[0], "id query");

			list = await (s.CreateQuery("select foo.TheFoo from foo in class Foo where foo.String='fizard'").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "query");
			Assert.AreSame(foo.TheFoo, list[0], "returned object");

			list = await (s.CreateQuery("from foo in class Foo where foo.Component.Subcomponent.Name='bar'").ListAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery("select foo.TheFoo from foo in class Foo where foo.TheFoo.id=?")
				.SetString(0, foo.TheFoo.Key).ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "by id query");
			Assert.AreSame(foo.TheFoo, list[0], "by id returned object");

			await (s.CreateQuery("from foo in class Foo where foo.TheFoo = ?").SetEntity(0, foo.TheFoo).ListAsync(CancellationToken.None));

			Assert.IsTrue(
				IsEmpty(await (s.CreateQuery("from bar in class Bar where bar.String='a string' or bar.String='a string'").EnumerableAsync(CancellationToken.None))
					));
			...
			enumerable = await (s.CreateQuery(
						...
						SetString(0, foo.TheFoo.Key).EnumerableAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery("from foo in class Foo where foo.String='from BoogieDown  -tinsel town  =!@#$^&*())'").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "single quotes");

			list = await (s.CreateQuery("from foo in class Foo where not foo.String='foo''bar'").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count, "single quotes");

			list = await (s.CreateQuery("from foo in class Foo where foo.Component.Glarch.Next is null").ListAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery(" from i in class Bar where i.Baz.Name='Bazza'").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count, "query many-to-one");

			if (DialectSupportsCountDistinct)
			{
				enumerable = await (s.CreateQuery("select count(distinct foo.TheFoo) from foo in class Foo").EnumerableAsync(CancellationToken.None));
				Assert.IsTrue(ContainsSingleObject(enumerable, (long) 2), "count"); // changed to Int64 (HQLFunction H3.2)
			}

			enumerable = await (s.CreateQuery("select count(foo.TheFoo.Boolean) from foo in class Foo").EnumerableAsync(CancellationToken.None));
			Assert.IsTrue(ContainsSingleObject(enumerable, (long) 2), "count"); // changed to Int64 (HQLFunction H3.2)

			enumerable = await (s.CreateQuery("select count(*), foo.Int from foo in class Foo group by foo.Int").EnumerableAsync(CancellationToken.None));
			enumerator = enumerable.GetEnumerator();
			Assert.IsTrue(enumerator.MoveNext());
			Assert.AreEqual(3L, (long) ((object[]) enumerator.Current)[0]);
			Assert.IsFalse(enumerator.MoveNext());

			enumerable = await (s.CreateQuery("select sum(foo.TheFoo.Int) from foo in class Foo").EnumerableAsync(CancellationToken.None));
			...
			enumerable = await (s.CreateQuery("select count(foo) from foo in class Foo where foo.id=?")
				.SetString(0, foo.Key).EnumerableAsync(CancellationToken.None));
			Assert.IsTrue(ContainsSingleObject(enumerable, (long) 1), "id query count");

			list = await (s.CreateQuery("from foo in class Foo where foo.Boolean = ?").SetBoolean(0, true).ListAsync(CancellationToken.None));

			list = await (s.CreateQuery("select new Foo(fo.X) from Fo fo").ListAsync(CancellationToken.None));
			list = await (s.CreateQuery("select new Foo(fo.Integer) from Foo fo").ListAsync(CancellationToken.None));
			...
			enumerable = await (s.CreateQuery("select new Foo(fo.X) from Foo fo").EnumerableAsync(CancellationToken.None));
			enumerator = enumerable.GetEnumerator();
			...
			list = await (s.CreateQuery("select foo.Long, foo.Component.Name, foo, foo.TheFoo from foo in class Foo").ListAsync(CancellationToken.None));
			...
			{
				list =
					await (s.CreateQuery("select avg(foo.Float), max(foo.Component.Name), count(distinct foo.id) from foo in class Foo").ListAsync(CancellationToken.None));
				Assert.IsTrue(list.Count > 0);
				foreach (object[] row in list)
				{
			...
			list = await (s.CreateQuery("select foo.Long, foo.Component, foo, foo.TheFoo from foo in class Foo").ListAsync(CancellationToken.None));
			Assert.IsTrue(list.Count > 0);
			foreach (object[] row in list)
			{
				Assert.IsTrue(row[0] is long);
			...
			await (s.SaveAsync(new Holder("ice T"), CancellationToken.None));
			await (s.SaveAsync(new Holder("ice cube"), CancellationToken.None));

			Assert.AreEqual(15, (await (s.CreateQuery("from o in class System.Object").ListAsync(CancellationToken.None))).Count);
			Assert.AreEqual(7, (await (s.CreateQuery("from n in class INamed").ListAsync(CancellationToken.None))).Count);
			Assert.IsTrue((await (s.CreateQuery("from n in class INamed where n.Name is not null").ListAsync(CancellationToken.None))).Count == 4);

			foreach (INamed named in await (s.CreateQuery("from n in class INamed").EnumerableAsync(CancellationToken.None)))
			{
				Assert.IsNotNull(named);
			}

			await (s.SaveAsync(new Holder("bar"), CancellationToken.None));
			enumerable = await (s.CreateQuery("from n0 in class INamed, n1 in class INamed where n0.Name = n1.Name").EnumerableAsync(CancellationToken.None));
			int cnt = 0;
			foreach (object[] row in enumerable)
			{
				if (row[0] != row[1])
			...
			//{
			Assert.IsTrue(cnt == 2);
			Assert.IsTrue((await (s.CreateQuery("from n0 in class INamed, n1 in class INamed where n0.Name = n1.Name").ListAsync(CancellationToken.None))).Count == 7);
			//}
			...
			foreach (object obj in await (s.CreateQuery("from o in class System.Object").EnumerableAsync(CancellationToken.None)))
			{
				c++;
			}
			Assert.IsTrue(c == 16);

			await (s.CreateQuery("select baz.Code, min(baz.Count) from baz in class Baz group by baz.Code").EnumerableAsync(CancellationToken.None));

			Assert.IsTrue(
				IsEmpty(
					await (s.CreateQuery(
						"selecT baz from baz in class Baz where baz.StringDateMap['foo'] is not null or baz.StringDateMap['bar'] = ?")
						.SetDateTime(0, DateTime.Today).EnumerableAsync(CancellationToken.None))));

			list = await (s.CreateQuery("select baz from baz in class Baz where baz.StringDateMap['now'] is not null").ListAsync(CancellationToken.None));
			...
			list = await (s.CreateQuery("select index(date) from Baz baz join baz.StringDateMap date").ListAsync(CancellationToken.None));
			...
			await (s.CreateQuery("from Baz baz inner join baz.CollectionComponent.Nested.Foos foo where foo.String is null").ListAsync(CancellationToken.None));
			...
			await (s.CreateQuery("from Foo foo join foo.TheFoo where foo.TheFoo in ('1','2','3')").ListAsync(CancellationToken.None));

			//if ( !(dialect is Dialect.HSQLDialect) )
			await (s.CreateQuery("from Foo foo left join foo.TheFoo where foo.TheFoo in ('1','2','3')").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.TheFoo from Foo foo where foo.TheFoo in ('1','2','3')").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.TheFoo.String from Foo foo where foo.TheFoo in ('1','2','3')").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.TheFoo.String from Foo foo where foo.TheFoo.String in ('1','2','3')").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.TheFoo.Long from Foo foo where foo.TheFoo.String in ('1','2','3')").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select count(*) from Foo foo where foo.TheFoo.String in ('1','2','3') or foo.TheFoo.Long in (1,2,3)").
				ListAsync(CancellationToken.None));
			await (s.CreateQuery("select count(*) from Foo foo where foo.TheFoo.String in ('1','2','3') group by foo.TheFoo.Long").ListAsync(CancellationToken.None));

			await (s.CreateQuery("from Foo foo1 left join foo1.TheFoo foo2 left join foo2.TheFoo where foo1.String is not null").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Foo foo1 left join foo1.TheFoo.TheFoo where foo1.String is not null").ListAsync(CancellationToken.None));
			await (s.CreateQuery(
				"from Foo foo1 left join foo1.TheFoo foo2 left join foo1.TheFoo.TheFoo foo3 where foo1.String is not null").ListAsync(CancellationToken.None));

			await (s.CreateQuery("select foo.Formula from Foo foo where foo.Formula > 0").ListAsync(CancellationToken.None));

			int len = (await (s.CreateQuery("from Foo as foo join foo.TheFoo as foo2 where foo2.id >'a' or foo2.id <'a'").ListAsync(CancellationToken.None))).Count;
			Assert.IsTrue(len == 2);

			await (s.DeleteAsync("from Holder", CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			txn = s.BeginTransaction();
			baz = (Baz) await (s.CreateQuery("from Baz baz left outer join fetch baz.ManyToAny").UniqueResultAsync(CancellationToken.None));
			...
			await (s.CreateQuery("from Baz baz join baz.ManyToAny").ListAsync(CancellationToken.None));
			Assert.IsTrue((await (s.CreateQuery("select baz from Baz baz join baz.ManyToAny a where index(a) = 0").ListAsync(CancellationToken.None))).Count == 1);

			FooProxy foop = (FooProxy) await (s.GetAsync(typeof(Foo), foo.Key, CancellationToken.None));
			Assert.IsTrue(foop == baz.ManyToAny[1]);

			barp.Baz = baz;
			Assert.IsTrue((await (s.CreateQuery("select bar from Bar bar where bar.Baz.StringDateMap['now'] is not null").ListAsync(CancellationToken.None))).Count ==
			              1);
			Assert.IsTrue(
(				await (s.CreateQuery(
					...
					.ListAsync(CancellationToken.None))).Count == 1);
			Assert.IsTrue(
(				await (s.CreateQuery(
			...
			list = await (s.CreateQuery("select foo.String, foo.Component, foo.id from Bar foo").ListAsync(CancellationToken.None));
			Assert.IsTrue(((FooComponent) ((object[]) list[0])[1]).Name == "foo");
			list = await (s.CreateQuery("select elements(baz.Components) from Baz baz").ListAsync(CancellationToken.None));
			Assert.IsTrue(list.Count == 2);
			list = await (s.CreateQuery("select bc.Name from Baz baz join baz.Components bc").ListAsync(CancellationToken.None));
			Assert.IsTrue(list.Count == 2);
			//list = s.CreateQuery("select bc from Baz baz join baz.components bc").List();

			await (s.CreateQuery("from Foo foo where foo.Integer < 10 order by foo.String").SetMaxResults(12).ListAsync(CancellationToken.None));

			await (s.DeleteAsync(barp, CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.DeleteAsync(foop.TheFoo, CancellationToken.None));
			await (s.DeleteAsync(foop, CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CascadeDeleteDetachedAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				baz = (Baz) await (s.GetAsync(typeof(Baz), baz.Code, CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				...
				Assert.IsFalse((await (s.CreateQuery("from Fee").EnumerableAsync(CancellationToken.None))).GetEnumerator().MoveNext());
			}
			...
			{
				baz = (Baz) await (s.GetAsync(typeof(Baz), baz.Code, CancellationToken.None));
				await (NHibernateUtil.InitializeAsync(baz.Fees, CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				await (s.DeleteAsync(baz, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
				Assert.IsTrue(IsEmpty(await (s.CreateQuery("from Fee").EnumerableAsync(CancellationToken.None))));
			}
		}
		...
		public async Task ForeignKeysAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				baz = (Baz) await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None));
				await (s.DeleteAsync(baz, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task NonlazyCollectionsAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(glarch2, CancellationToken.None));
				glarchId = await (s.SaveAsync(glarch1, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				loadedGlarch = (Glarch) await (s.GetAsync(typeof(Glarch), glarchId, CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				await (s.DeleteAsync("from Glarch", CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task ReuseDeletedCollectionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				await (s.UpdateAsync(baz2, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}

			using (ISession s = OpenSession())
			{
				baz2 = (Baz) await (s.LoadAsync(typeof(Baz), baz2.Code, CancellationToken.None));
		...
		public async Task PropertyRefAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				h2.Name = "bar";
				h.OtherHolder = h2;
				hid = await (s.SaveAsync(h, CancellationToken.None));

				Qux q = new Qux();
				q.Holder = h2;
				qid = await (s.SaveAsync(q, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}

			using (ISession s = OpenSession())
			{
				Holder h = (Holder) await (s.LoadAsync(typeof(Holder), hid, CancellationToken.None));
				Assert.AreEqual(h.Name, "foo");
				Assert.AreEqual(h.OtherHolder.Name, "bar");
				object[] res =
					(object[]) (await (s.CreateQuery("from Holder h join h.OtherHolder oh where h.OtherHolder.Name = 'bar'").ListAsync(CancellationToken.None)))[0];
				Assert.AreSame(h, res[0]);

				Qux q = (Qux) await (s.GetAsync(typeof(Qux), qid, CancellationToken.None));
		...
		public async Task QueryCollectionOfValuesAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				if (Dialect.SupportsSubSelects)
				{
					await ((await (s.CreateFilterAsync(baz.FooArray, "where size(this.Bytes) > 0", CancellationToken.None))).ListAsync(CancellationToken.None));
					await ((await (s.CreateFilterAsync(baz.FooArray, "where 0 in elements(this.Bytes)", CancellationToken.None))).ListAsync(CancellationToken.None));
				}
				await (s.FlushAsync(CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.CreateQuery("from Baz baz join baz.FooSet foo join foo.TheFoo.TheFoo foo2 where foo2.String = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from Baz baz join baz.FooArray foo join foo.TheFoo.TheFoo foo2 where foo2.String = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from Baz baz join baz.StringDateMap date where index(date) = 'foo'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from Baz baz join baz.TopGlarchez g where index(g) = 'A'").ListAsync(CancellationToken.None));
				await (s.CreateQuery("select index(g) from Baz baz join baz.TopGlarchez g").ListAsync(CancellationToken.None));

				Assert.AreEqual(3, (await (s.CreateQuery("from Baz baz left join baz.StringSet").ListAsync(CancellationToken.None))).Count);
				Baz baz = (Baz) (await (s.CreateQuery("from Baz baz join baz.StringSet str where str='foo'").ListAsync(CancellationToken.None)))[0];
				Assert.IsFalse(NHibernateUtil.IsInitialized(baz.StringSet));
				baz = (Baz) (await (s.CreateQuery("from Baz baz left join fetch baz.StringSet").ListAsync(CancellationToken.None)))[0];
				Assert.IsTrue(NHibernateUtil.IsInitialized(baz.StringSet));
				Assert.AreEqual(1, (await (s.CreateQuery("from Baz baz join baz.StringSet string where string='foo'").ListAsync(CancellationToken.None))).Count);
				Assert.AreEqual(1, (await (s.CreateQuery("from Baz baz inner join baz.Components comp where comp.Name='foo'").ListAsync(CancellationToken.None))).Count);
				//IList bss = s.CreateQuery("select baz, ss from Baz baz inner join baz.StringSet ss").List();
				await (s.CreateQuery("from Glarch g inner join g.FooComponents comp where comp.Fee is not null").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from Glarch g inner join g.FooComponents comp join comp.Fee fee where fee.Count > 0").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from Glarch g inner join g.FooComponents comp where comp.Fee.Count is not null").ListAsync(CancellationToken.None));

				await (s.DeleteAsync(baz, CancellationToken.None));
				//s.delete("from Glarch g");
				await (s.DeleteAsync(await (s.GetAsync(typeof(Glarch), gid, CancellationToken.None)), CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task BatchLoadAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(baz3, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				baz = (Baz) await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None));
				baz2 = (Baz) await (s.LoadAsync(typeof(Baz), baz2.Code, CancellationToken.None));
				baz3 = (Baz) await (s.LoadAsync(typeof(Baz), baz3.Code, CancellationToken.None));
				...
				await (s.DeleteAsync(baz, CancellationToken.None));
				await (s.DeleteAsync(baz2, CancellationToken.None));
				await (s.DeleteAsync(baz3, CancellationToken.None));
		...
		public async Task FetchInitializedCollectionAsync()
		{
			...
			await (s.FlushAsync(CancellationToken.None));
			fooBag = baz.FooBag;
			await (s.CreateQuery("from Baz baz left join fetch baz.FooBag").ListAsync(CancellationToken.None));
		...
		public async Task LateCollectionAddAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				l.Add("foo");
				id = await (s.SaveAsync(baz, CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				Baz baz = (Baz) await (s.LoadAsync(typeof(Baz), id, CancellationToken.None));
		...
		public async Task UpdateAsync()
		{
			...
			s = OpenSession();
			FooProxy foo2 = (FooProxy) await (s.LoadAsync(typeof(Foo), foo.Key, CancellationToken.None));
			foo2.String = "dirty";
			foo2.Boolean = false;
			...
			s = OpenSession();
			Foo foo3 = new Foo();
			await (s.LoadAsync(foo3, foo.Key, CancellationToken.None));
			Assert.IsTrue(foo2.EqualsFoo(foo3), "update");
			await (s.DeleteAsync(foo3, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ListRemoveAsync()
		{
			using (ISession s = OpenSession())
			{
				...
				await (s.EvictAsync(b, CancellationToken.None));
				await (s.RefreshAsync(b, CancellationToken.None));
				...
				await (s.DeleteAsync(b, CancellationToken.None));
				await (s.DeleteAsync("from Fee", CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task FetchInitializedCollectionDupeAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				Baz baz = (Baz) await (s.LoadAsync(typeof(Baz), bazCode, CancellationToken.None));
				object bag = baz.FooBag;
				Assert.IsFalse(NHibernateUtil.IsInitialized(bag));
				await (s.CreateQuery("from Baz baz left join fetch baz.FooBag").ListAsync(CancellationToken.None));
		...
		public async Task FetchListAsync()
		{
			...
			baz.Fees = list;
			var result = await (s.CreateQuery("from Foo foo, Baz baz left join fetch baz.Fees").ListAsync(CancellationToken.None));
			Assert.IsTrue(NHibernateUtil.IsInitialized(((Baz)((object[])result[0])[1]).Fees));
			await (s.DeleteAsync(foo, CancellationToken.None));
			await (s.DeleteAsync(foo2, CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task BagOneToManyAsync()
		{
			...
			list.RemoveAt(1);
			await (s.DeleteAsync(toDelete, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task QueryLockModeAsync()
		{
			...
			object[] result = (object[]) (await (q.ListAsync(CancellationToken.None)))[0];
			Assert.IsNotNull(bar.Bytes);
			...
			Assert.IsNotNull(bar.Bytes);
			await (s.CreateQuery("from Foo foo").ListAsync(CancellationToken.None));
			Assert.IsNotNull(bar.Bytes);
			Assert.AreEqual(LockMode.None, s.GetCurrentLockMode(b));
			...
			await (s.EvictAsync(baz, CancellationToken.None));
			...
			Assert.AreEqual(LockMode.None, s.GetCurrentLockMode(b));
			await (s.DeleteAsync(await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None)), CancellationToken.None));
			Assert.AreEqual(LockMode.None, s.GetCurrentLockMode(b));
			...
			result = (object[]) (await (q.ListAsync(CancellationToken.None)))[0];
		...
		public async Task ManyToManyBagAsync()
		{
			ISession s = OpenSession();
			Baz baz = new Baz();
			object id = await (s.SaveAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			baz = (Baz) await (s.LoadAsync(typeof(Baz), id, CancellationToken.None));
		...
		public async Task IdBagAsync()
		{
			...
			l.RemoveAt(3);
			await (s.DeleteAsync(removedObject, CancellationToken.None));
		...
		public async Task ForceOuterJoinAsync()
		{
			...
			f.Component = fc;
			f.TheFoo = f2;
			await (s.SaveAsync(f2, CancellationToken.None));
			object id = await (s.SaveAsync(f, CancellationToken.None));
			object gid = s.GetIdentifier(f.Component.Glarch);
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			f = (FooProxy) await (s.LoadAsync(typeof(Foo), id, CancellationToken.None));
			...
			await (s.DeleteAsync(f, CancellationToken.None));
			await (s.DeleteAsync(f.TheFoo, CancellationToken.None));
			await (s.DeleteAsync(f.Component.Glarch, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		{
			ISession s = OpenSession();
			object id = await (s.SaveAsync(new Baz(), CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			s = OpenSession();
			Baz baz = (Baz) await (s.LoadAsync(typeof(Baz), id, CancellationToken.None));
		...
		public async Task OneToOneGeneratorAsync()
		{
			...
			object yId = await (s.SaveAsync(y, CancellationToken.None));
			object xId = await (s.SaveAsync(x, CancellationToken.None));
			...
			await (s.SaveAsync(y, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			...
			x.Y = y;
			y.TheX = x;
			xId = await (s.SaveAsync(x, CancellationToken.None));
			...
			Assert.IsTrue(s.Contains(y) && s.Contains(x));
			await (s.DeleteAsync("from X x", CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			y = (Y) await (s.LoadAsync(typeof(Y), y.Id, CancellationToken.None));
			Assert.IsNull(y.X, "y does not need an X");
			await (s.DeleteAsync(y, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task LimitAsync()
		{
			...
			for (int i = 0; i < 10; i++)
			{
				await (s.SaveAsync(new Foo(), CancellationToken.None));
			}
			...
			Assert.AreEqual(10, await (s.DeleteAsync("from Foo foo", CancellationToken.None)));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CustomAsync()
		{
			...
			ISession s = OpenSession();
			object gid = await (s.SaveAsync(g, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			g = (Glarch) (await (s.CreateQuery("from Glarch g where g.Multiple.glarch=g and g.Multiple.count=12").ListAsync(CancellationToken.None)))[0];
			...
			s = OpenSession();
			g = (GlarchProxy) await (s.LoadAsync(typeof(Glarch), gid, CancellationToken.None));
			Assert.IsNotNull(g.Multiple);
			Assert.AreEqual(12, g.Multiple.count);
			Assert.AreSame(g, g.Multiple.glarch);
			await (s.DeleteAsync(g, CancellationToken.None));
		...
		public async Task NamedParamsAsync()
		{
			...
			ITransaction txn = s.BeginTransaction();
			await (s.SaveAsync(baz, CancellationToken.None));
			await (s.SaveAsync(bar2, CancellationToken.None));

			IList list =
				await (s.CreateQuery("from Bar bar left join bar.Baz baz left join baz.CascadingBars b where bar.Name like 'Bar %'").ListAsync(CancellationToken.None));
			object row = list[0];
			...
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.DeleteAsync(bar2, CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task DynaAsync()
		{
			...
			GlarchProxy g = new Glarch();
			g.Name = "G";
			object id = await (s.SaveAsync(g, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			g = (GlarchProxy) await (s.LoadAsync(typeof(Glarch), id, CancellationToken.None));
		...
		public async Task FindByCriteriaAsync()
		{
			...
			list = await (s.CreateCriteria(typeof(Foo)).SetMaxResults(0).ListAsync(CancellationToken.None));
			...
			f.TheFoo = new Foo();
			await (s.SaveAsync(f.TheFoo, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task CollectionWhereAsync()
		{
			...
			ISession s = OpenSession();
			await (s.SaveAsync(foo1, CancellationToken.None));
			await (s.SaveAsync(foo2, CancellationToken.None));
			baz.FooArray = arr;
			await (s.SaveAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			...
			Assert.AreEqual(1, (await (s.CreateQuery("from Baz baz join baz.FooArray foo").ListAsync(CancellationToken.None))).Count);
			Assert.AreEqual(2, (await (s.CreateQuery("from Foo foo").ListAsync(CancellationToken.None))).Count);
			Assert.AreEqual(1, (await ((await (s.CreateFilterAsync(baz.FooArray, "", CancellationToken.None))).ListAsync(CancellationToken.None))).Count);

			await (s.DeleteAsync("from Foo foo", CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
		...
		public async Task ComponentParentAsync()
		{
			...
			bar = (BarProxy) await (s.LoadAsync(typeof(Bar), bar.Key, CancellationToken.None));
			await (s.LoadAsync(baz, baz.Code, CancellationToken.None));

			Assert.AreEqual(bar, bar.BarComponent.Parent);
			Assert.IsTrue(baz.Components[0].Baz == baz && baz.Components[1].Baz == baz);

			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.DeleteAsync(bar, CancellationToken.None));
		...
		public async Task CollectionCacheAsync()
		{
			...
			s = OpenSession();
			await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task AssociationIdAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				{
					bar = new Bar();
					id = (string) await (s.SaveAsync(bar, CancellationToken.None));
					more = new MoreStuff();
					...
					await (s.SaveAsync(more, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					...
					await (s.CreateQuery(hqlString).ListAsync(CancellationToken.None));

					hqlString = "from s in class Stuff where s.Foo > '0' order by s.Foo";
					results = await (s.CreateQuery(hqlString).ListAsync(CancellationToken.None));
					Assert.AreEqual(1, results.Count);
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					foo = (FooProxy) await (s.LoadAsync(typeof(Foo), id, CancellationToken.None));
					await (s.LoadAsync(more, more, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction t = s.BeginTransaction())
				{
					...
					await (s.LoadAsync(stuff, stuff, CancellationToken.None));

					Assert.AreEqual("More Stuff", stuff.MoreStuff.Name);
					await (s.DeleteAsync("from ms in class MoreStuff", CancellationToken.None));
					await (s.DeleteAsync("from foo in class Foo", CancellationToken.None));
		...
		public async Task CascadeSaveAsync()
		{
			...
			Assert.IsTrue(IsEmpty(await (s.CreateQuery("from fee in class Fee").EnumerableAsync(CancellationToken.None))));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CompositeKeyPathExpressionsAsync()
		{
			...
			await (s.CreateQuery(hql).ListAsync(CancellationToken.None));
		...
		public async Task CollectionsInSelectAsync()
		{
			...
			await (s.SaveAsync(foos[1], CancellationToken.None));
			Baz baz = new Baz();
			baz.SetDefaults();
			baz.FooArray = foos;
			await (s.SaveAsync(baz, CancellationToken.None));
			...
			IList list = await (s.CreateQuery("select new Result(foo.String, foo.Long, foo.Integer) from foo in class Foo").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count);
			...
			await (s.CreateQuery("select max( elements(bar.Baz.FooArray) ) from Bar as bar").ListAsync(CancellationToken.None));
			// the following test is disable for databases with no subselects... also for Interbase (not sure why) - comment from h2.0.3
			if (Dialect.SupportsSubSelects)
			{
				await (s.CreateQuery("select count(*) from Baz as baz where 1 in indices(baz.FooArray)").ListAsync(CancellationToken.None));
				await (s.CreateQuery("select count(*) from Bar as bar where 'abc' in elements(bar.Baz.FooArray)").ListAsync(CancellationToken.None));
				await (s.CreateQuery("select count(*) from Bar as bar where 1 in indices(bar.Baz.FooArray)").ListAsync(CancellationToken.None));
				await (s.CreateQuery(
					"select count(*) from Bar as bar where '1' in (from bar.Component.Glarch.ProxyArray g where g.Name='foo')").ListAsync(CancellationToken.None));
			...
			list =
				await (s.CreateQuery("from Baz baz left join baz.FooToGlarch join fetch baz.FooArray foo left join fetch foo.TheFoo").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count);
			Assert.AreEqual(2, ((object[]) list[0]).Length);

			list =
				await (s.CreateQuery(
					"select baz.Name from Bar bar inner join bar.Baz baz inner join baz.FooSet foo where baz.Name = bar.String").ListAsync(CancellationToken.None));
			await (s.CreateQuery(
				"SELECT baz.Name FROM Bar AS bar INNER JOIN bar.Baz AS baz INNER JOIN baz.FooSet AS foo WHERE baz.Name = bar.String")
				.ListAsync(CancellationToken.None));

			await (s.CreateQuery(
				"select baz.Name from Bar bar join bar.Baz baz left outer join baz.FooSet foo where baz.Name = bar.String").ListAsync(CancellationToken.None));

			await (s.CreateQuery("select baz.Name from Bar bar join bar.Baz baz join baz.FooSet foo where baz.Name = bar.String").ListAsync(CancellationToken.None));
			await (s.CreateQuery("SELECT baz.Name FROM Bar AS bar join bar.Baz AS baz join baz.FooSet AS foo WHERE baz.Name = bar.String").ListAsync(CancellationToken.None));

			await (s.CreateQuery(
				"select baz.Name from Bar bar left join bar.Baz baz left join baz.FooSet foo where baz.Name = bar.String").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.String from Bar bar left join bar.Baz.FooSet foo where bar.String = foo.String").ListAsync(CancellationToken.None));

			await (s.CreateQuery(
				"select baz.Name from Bar bar left join bar.Baz baz left join baz.FooArray foo where baz.Name = bar.String").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo.String from Bar bar left join bar.Baz.FooArray foo where bar.String = foo.String").ListAsync(CancellationToken.None));
			...
			await (s.CreateQuery(
				...
				.ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo from bar in class Bar inner join bar.Baz as baz inner join baz.FooSet as foo").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo from bar in class Bar inner join bar.Baz.FooSet as foo").ListAsync(CancellationToken.None));

			await (s.CreateQuery(
				"select bar.String, foo.String from bar in class Bar join bar.Baz as baz, elements(baz.FooSet) as foo where baz.Name = 'name'")
				.ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo from bar in class Bar join bar.Baz as baz join baz.FooSet as foo").ListAsync(CancellationToken.None));
			await (s.CreateQuery("select foo from bar in class Bar join bar.Baz.FooSet as foo").ListAsync(CancellationToken.None));

			Assert.AreEqual(1, (await (s.CreateQuery("from Bar bar join bar.Baz.FooArray foo").ListAsync(CancellationToken.None))).Count);

			Assert.AreEqual(0, (await (s.CreateQuery("from bar in class Bar, foo in elements(bar.Baz.FooSet)").ListAsync(CancellationToken.None))).Count);

			Assert.AreEqual(1, (await (s.CreateQuery("from bar in class Bar, foo in elements( bar.Baz.FooArray )").ListAsync(CancellationToken.None))).Count);

			await (s.DeleteAsync(bar, CancellationToken.None));

			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.DeleteAsync(baz2, CancellationToken.None));
			await (s.DeleteAsync(foos[1], CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task NewFlushingAsync()
		{
			...
			IEnumerator enumer = (await (s.CreateQuery("from baz in class Baz").EnumerableAsync(CancellationToken.None))).GetEnumerator(); // no flush
			Assert.IsTrue(enumer.MoveNext());
			Assert.AreSame(baz, enumer.Current);

			enumer = (await (s.CreateQuery("select elements(baz.StringArray) from baz in class Baz").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			...
			await (s.CreateQuery("from baz in class Baz").EnumerableAsync(CancellationToken.None)); // no flush
			...
			enumer = (await (s.CreateQuery("from foo in class Foo").EnumerableAsync(CancellationToken.None))).GetEnumerator(); // no flush
			Assert.IsFalse(enumer.MoveNext());

			enumer = (await (s.CreateQuery("select elements(baz.StringList) from baz in class Baz").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			...
			await (s.CreateQuery("select elements(baz.StringArray) from baz in class Baz").EnumerableAsync(CancellationToken.None)); //no flush
...
(			
			await (s.CreateQuery("from foo in class Foo").EnumerableAsync(CancellationToken.None))).GetEnumerator(); //no flush
		...
		public async Task PersistCollectionsAsync()
		{
			ISession s = OpenSession();
			ITransaction txn = s.BeginTransaction();
			IEnumerator enumer = (await (s.CreateQuery("select count(*) from b in class Bar").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			enumer.MoveNext();
			...
			s = OpenSession();
			txn = s.BeginTransaction();
			baz = (Baz) ((object[]) (await (s.CreateQuery("select baz, baz from baz in class NHibernate.DomainModel.Baz").ListAsync(CancellationToken.None)))[0])[1];
			...
			list = await (s.CreateQuery("select foo from baz in class NHibernate.DomainModel.Baz, foo in elements(baz.FooSet)").ListAsync(CancellationToken.None));
			...
			s = OpenSession();
			txn = s.BeginTransaction();
			baz = (Baz)(await (s.CreateQuery("select baz from baz in class NHibernate.DomainModel.Baz order by baz").ListAsync(CancellationToken.None)))[0];
			...
			GlarchProxy g = new Glarch();
			await (s.SaveAsync(g, CancellationToken.None));
			baz.TopGlarchez['G'] = g;
			...
			using (txn = s.BeginTransaction())
			{
				...
				await (txn2.CommitAsync(CancellationToken.None));
				s2.Close();
				...
				Glarch g2 = new Glarch();
				await (s.SaveAsync(g2, CancellationToken.None));
			...
			baz = (Baz) await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.DeleteAsync(baz.TopGlarchez['G'], CancellationToken.None));
			await (s.DeleteAsync(baz.TopGlarchez['H'], CancellationToken.None));
			...
			Assert.AreEqual(1, rows);
			Assert.AreEqual(2, await (s.DeleteAsync("from bar in class NHibernate.DomainModel.Bar", CancellationToken.None)));
			...
			for (int i = 0; i < arr.Length; i++)
			{
				if (arr[i] != null)
				{
					await (s.DeleteAsync(arr[i], CancellationToken.None));
				}
			}

			try
			{
				await (s.LoadAsync(typeof(Qux), (long) 666, CancellationToken.None)); //nonexistent
			}
			...
			Assert.AreEqual(1, await (s.DeleteAsync("from g in class Glarch", CancellationToken.None)), "Delete('from g in class Glarch')");
			await (txn.CommitAsync(CancellationToken.None));
			s.Disconnect();
			...
			Qux nonexistentQux = (Qux) await (s.LoadAsync(typeof(Qux), (long) 666, CancellationToken.None)); //nonexistent
		...
		public async Task SaveFlushAsync()
		{
			ISession s = OpenSession();
			Fee fee = new Fee();
			await (s.SaveAsync(fee, "key", CancellationToken.None));
			fee.Fi = "blah";
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			fee = (Fee) await (s.LoadAsync(typeof(Fee), fee.Key, CancellationToken.None));
			Assert.AreEqual("blah", fee.Fi);
			Assert.AreEqual("key", fee.Key);
			await (s.DeleteAsync(fee, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CreateUpdateAsync()
		{
			...
			s = OpenSession();
			Foo foo2 = new Foo();
			await (s.LoadAsync(foo2, foo.Key, CancellationToken.None));
			...
			s = OpenSession();
			foo = new Foo();
			await (s.SaveAsync(foo, "assignedid", CancellationToken.None));
			foo.String = "dirty";
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			await (s.LoadAsync(foo2, "assignedid", CancellationToken.None));
			Assert.IsTrue(foo.EqualsFoo(foo2), "create-update");
			await (s.DeleteAsync(foo2, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task UpdateCollectionsAsync()
		{
			...
			await (s.SaveAsync(f2, CancellationToken.None));
			await (s.SaveAsync(f3, CancellationToken.None));
			...
			s = OpenSession();
			await (s.SaveOrUpdateAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			Holder h = (Holder) await (s.LoadAsync(typeof(Holder), baz.Id, CancellationToken.None));
			...
			s = OpenSession();
			await (s.SaveOrUpdateAsync(baz, CancellationToken.None));
			await (s.DeleteAsync("from f in class NHibernate.DomainModel.Foo", CancellationToken.None));
			baz.Ones.Remove(o);
			await (s.DeleteAsync("from o in class NHibernate.DomainModel.One", CancellationToken.None));
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task LoadAsync()
		{
			...
			s = OpenSession();
			q = (Qux) await (s.LoadAsync(typeof(Qux), q.Key, CancellationToken.None));
			b = (BarProxy) await (s.LoadAsync(typeof(Foo), b.Key, CancellationToken.None));
			...
			BarProxy b2 = (BarProxy) await (s.LoadAsync(typeof(Bar), tempKey, CancellationToken.None));
			Qux q2 = (Qux) await (s.LoadAsync(typeof(Qux), q.Key, CancellationToken.None));
			Assert.AreSame(q, q2, "loaded same Qux");
			Assert.AreSame(b, b2, "loaded same BarProxy");
			await (s.DeleteAsync(q2, CancellationToken.None));
			await (s.DeleteAsync(b2, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CallbackAsync()
		{
			...
			await (s.SaveAsync(q, CancellationToken.None));
			q.Child = (new Qux("1"));
			await (s.SaveAsync(q.Child, CancellationToken.None));
			...
			Qux q3 = new Qux("3");
			q.Child.Child = q3;
			await (s.SaveAsync(q3, CancellationToken.None));

			Qux q4 = new Qux("4");
			q4.Child = q3;

			await (s.SaveAsync(q4, CancellationToken.None));
			await (s.SaveAsync(q2, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			IList l = await (s.CreateQuery("from q in class NHibernate.DomainModel.Qux").ListAsync(CancellationToken.None));
			Assert.AreEqual(5, l.Count);

			await (s.DeleteAsync(l[0], CancellationToken.None));
			await (s.DeleteAsync(l[1], CancellationToken.None));
			await (s.DeleteAsync(l[2], CancellationToken.None));
			await (s.DeleteAsync(l[3], CancellationToken.None));
			await (s.DeleteAsync(l[4], CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task PolymorphismAsync()
		{
			...
			s = OpenSession();
			FooProxy foo = (FooProxy) await (s.LoadAsync(typeof(Foo), bar.Key, CancellationToken.None));
		...
		public async Task RemoveContainsAsync()
		{
			...
			Baz baz2 = (Baz) await (s.LoadAsync(typeof(Baz), baz.Code, CancellationToken.None));
		...
		public async Task CollectionOfSelfAsync()
		{
			...
			bar.Abstracts = null;
			s = OpenSession();
			await (s.LoadAsync(bar, bar.Key, CancellationToken.None));
		...
		public async Task FindAsync()
		{
			...
			foo.String = "foo bar";
			await (s.SaveAsync(new Foo(), CancellationToken.None));
			await (s.SaveAsync(new Bar(), CancellationToken.None));

			IList list1 =
				await (s.CreateQuery("select foo from foo in class NHibernate.DomainModel.Foo where foo.String='foo bar'").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list1.Count, "find size");
			Assert.AreSame(foo, list1[0], "find ==");

			IList list2 = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo order by foo.String, foo.Date").ListAsync(CancellationToken.None));
			Assert.AreEqual(4, list2.Count, "find size");
			list1 = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.class='B'").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list1.Count, "class special property");
			list1 =
				await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.class=NHibernate.DomainModel.Bar").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list1.Count, "class special property");
			list1 = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.class=Bar").ListAsync(CancellationToken.None));
			list2 =
				await (s.CreateQuery(
			...
			await (s.SaveAsync(t, CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			txn = s.BeginTransaction();
			list1 = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.String='foo bar'").ListAsync(CancellationToken.None));
			...
			list2 = await (s.CreateQuery("select foo from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None));
			Assert.AreEqual(5, list2.Count, "find count");
			IList list3 = await (s.CreateQuery("from bar in class NHibernate.DomainModel.Bar where bar.BarString='bar bar'").ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list3.Count, "find count");
			Assert.IsTrue(list2.Contains(list1[0]) && list2.Contains(list2[0]), "find same instance");
			Assert.AreEqual(1, (await (s.CreateQuery("from t in class NHibernate.DomainModel.Trivial").ListAsync(CancellationToken.None))).Count);
			await (s.DeleteAsync("from t in class NHibernate.DomainModel.Trivial", CancellationToken.None));

			list2 =
				await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.Date = ?").SetDateTime(0,
				                                                                                             new DateTime(1970, 01,
				                                                                                                          01)).ListAsync(CancellationToken.None));
			Assert.AreEqual(4, list2.Count, "find by date");
			IEnumerator enumer = list2.GetEnumerator();
			while (enumer.MoveNext())
			{
				await (s.DeleteAsync(enumer.Current, CancellationToken.None));
			}

			list2 = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None));
		...
		public async Task DeleteRecursiveAsync()
		{
			...
			x.TheFoo = y;
			y.TheFoo = x;
			await (s.SaveAsync(x, CancellationToken.None));
			await (s.SaveAsync(y, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			await (s.DeleteAsync(y, CancellationToken.None));
			await (s.DeleteAsync(x, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ReachabilityAsync()
		{
			...
			ISession s = OpenSession();
			Baz baz1 = new Baz();
			await (s.SaveAsync(baz1, CancellationToken.None));
			Baz baz2 = new Baz();
			await (s.SaveAsync(baz2, CancellationToken.None));
			...
			s = OpenSession();
			baz2 = (Baz) await (s.LoadAsync(typeof(Baz), baz2.Code, CancellationToken.None));
			baz1 = (Baz) await (s.LoadAsync(typeof(Baz), baz1.Code, CancellationToken.None));
			...
			{
				await (s.DeleteAsync((FooProxy) obj, CancellationToken.None));
			}

			await (s.DeleteAsync(baz1, CancellationToken.None));
			await (s.DeleteAsync(baz2, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			await (s.DeleteAsync(baz2.FooArray[0], CancellationToken.None));
			await (s.DeleteAsync(baz2.FooArray[2], CancellationToken.None));
			await (s.DeleteAsync(baz1, CancellationToken.None));
			await (s.DeleteAsync(baz2, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task PersistentLifecycleAsync()
		{
			...
			s = OpenSession();
			Assert.AreEqual(0, (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None))).Count, "subdeletion");
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task EnumerableAsync()
		{
			...
			{
				Qux q = new Qux();
				object qid = await (s.SaveAsync(q, CancellationToken.None));
			...
			s = OpenSession();
			IEnumerator enumer =
(				await (s.CreateQuery("from q in class NHibernate.DomainModel.Qux where q.Stuff is null").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			int count = 0;
			while (enumer.MoveNext())
			{
			...
			Assert.AreEqual(8,
			                await (s.DeleteAsync("from q in class NHibernate.DomainModel.Qux where q.Stuff=?", "foo", NHibernateUtil.String, CancellationToken.None)),
			                "delete by query");
			...
			s = OpenSession();
			enumer = (await (s.CreateQuery("from q in class NHibernate.DomainModel.Qux").EnumerableAsync(CancellationToken.None))).GetEnumerator();
		...
		public async Task EnumerableDisposableAsync()
		{
			...
			for (long i = 0L; i < 10L; i++)
			{
				...
				simple.Count = (int) i;
				await (s.SaveAsync(simple, i, CancellationToken.None));
			...
			s = OpenSession();
			ITransaction t = s.BeginTransaction();
			Simple simp = (Simple) await (s.LoadAsync(typeof(Simple), 8L, CancellationToken.None));
			...
			IEnumerable enumer = await (s.CreateQuery("select s.Count from Simple as s").EnumerableAsync(CancellationToken.None));
			//int count = 0;
			...
			s = OpenSession();
			enumer = await (s.CreateQuery("from Simple").EnumerableAsync(CancellationToken.None));
		...
		public async Task VersioningAsync()
		{
			...
			{
				g = (GlarchProxy) await (s.LoadAsync(typeof(Glarch), gid, CancellationToken.None));
				await (s.LockAsync(g, LockMode.Upgrade, CancellationToken.None));
				g2 = (GlarchProxy) await (s.LoadAsync(typeof(Glarch), g2id, CancellationToken.None));
				...
				g.Name = "foo";
				Assert.IsTrue(
(					await (s.CreateQuery("from g in class Glarch where g.Version=3").ListAsync(CancellationToken.None))).Count == 1,
					"find by version"
					);
				g.Name = "bar";
				await (txn.CommitAsync(CancellationToken.None));
		...
		*/
		...
		public async Task RecursiveLoadAsync()
		{
			...
			await (s.SaveAsync(last, CancellationToken.None));
			last.Order = 0;
			for (int i = 0; i < 5; i++)
			{
				GlarchProxy next = new Glarch();
				await (s.SaveAsync(next, CancellationToken.None));
				last.Next = next;
				last = next;
				last.Order = (short) (i + 1);
			}

			IEnumerator enumer = (await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			while (enumer.MoveNext())
			{
				object objTemp = enumer.Current;
			}

			IList list = await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch").ListAsync(CancellationToken.None));
			...
			s = OpenSession();
			txn = s.BeginTransaction();
			list = await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch").ListAsync(CancellationToken.None));
			Assert.AreEqual(6, list.Count, "recursive iter");
			list = await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch where g.Next is not null").ListAsync(CancellationToken.None));
			...
			txn = s.BeginTransaction();
			enumer =
(				await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch order by g.Order asc").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			while (enumer.MoveNext())
			{
			...
			await (s.DeleteAsync("from NHibernate.DomainModel.Glarch as g", CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
			...
			await (s.SaveAsync(flast, CancellationToken.None));
			for (int i = 0; i < 5; i++)
			{
			...
			enumer = (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			while (enumer.MoveNext())
			{
				object objTemp = enumer.Current;
			}

			list = await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None));
			Assert.AreEqual(6, list.Count, "recursive find");
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
			...
			txn = s.BeginTransaction();
			enumer =
(				await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo order by foo.String asc").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			...
			await (s.DeleteAsync("from NHibernate.DomainModel.Foo as foo", CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
...
(					await (s.CreateQuery(
						...
						.EnumerableAsync(CancellationToken.None))).GetEnumerator();
...
(				await (s.CreateQuery(
			...
			while (enumer.MoveNext())
			{
				...
				await (s.DeleteAsync(pnc[0], CancellationToken.None));
				await (s.DeleteAsync(pnc[1], CancellationToken.None));
				deletions++;
			}
		...
		public async Task DeleteTransientAsync()
		{
			...
			ISession s = OpenSession();
			ITransaction tx = s.BeginTransaction();
			await (s.SaveAsync(fee, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			fee.Count = 123;
			await (tx.CommitAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			tx = s.BeginTransaction();
			Assert.AreEqual(0, (await (s.CreateQuery("from fee in class Fee").ListAsync(CancellationToken.None))).Count);
			await (tx.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task DeleteUpdatedTransientAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				{
					await (s.SaveAsync(fee, CancellationToken.None));
					await (s.SaveAsync(fee2, CancellationToken.None));
					await (s.FlushAsync(CancellationToken.None));
					fee.Count = 123;
					await (tx.CommitAsync(CancellationToken.None));
				}
			...
			using (ISession s = OpenSession())
			{
				using (ITransaction tx = s.BeginTransaction())
				{
					await (s.UpdateAsync(fee, CancellationToken.None));
					//fee2.AnotherFee = null;
					await (s.UpdateAsync(fee2, CancellationToken.None));
					await (s.DeleteAsync(fee, CancellationToken.None));
					await (s.DeleteAsync(fee2, CancellationToken.None));
					await (tx.CommitAsync(CancellationToken.None));
				}
			}
		...
		public async Task UpdateOrderAsync()
		{
			...
			{
				fee1 = new Fee();
				await (s.SaveAsync(fee1, CancellationToken.None));
				...
				await (s.SaveAsync(fee3, CancellationToken.None));
				await (s.SaveAsync(fee2, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				...
				fee2.Count = 20;
				fee3.Count = 30;
				await (s.UpdateAsync(fee1, CancellationToken.None));
				await (s.UpdateAsync(fee2, CancellationToken.None));
				await (s.UpdateAsync(fee3, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
				await (s.DeleteAsync(fee1, CancellationToken.None));
				await (s.DeleteAsync(fee2, CancellationToken.None));
				await (s.DeleteAsync(fee3, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		...
		public async Task UpdateFromTransientAsync()
		{
			...
			fee1.Fi = "changed";
			s = OpenSession();
			await (s.SaveOrUpdateAsync(fee1, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			fee1 = (Fee) await (s.LoadAsync(typeof(Fee), fee1.Key, CancellationToken.None));
			Assert.AreEqual("changed", fee1.Fi, "updated from transient");
			Assert.IsNotNull(fee1.Qux, "unsaved-value");
			await (s.DeleteAsync(fee1.Qux, CancellationToken.None));
			fee1.Qux = null;
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			await (s.SaveOrUpdateAsync(fee2, CancellationToken.None));
			await (s.UpdateAsync(fee1, fee1.Key, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			Fee fee = new Fee();
			await (s.LoadAsync(fee, fee2.Key, CancellationToken.None));
			fee1 = (Fee) await (s.LoadAsync(typeof(Fee), fee1.Key, CancellationToken.None));
			...
			fee1.TheFee = null;
			s = OpenSession();
			await (s.SaveOrUpdateAsync(fee, CancellationToken.None));
			await (s.SaveOrUpdateAsync(fee1, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			await (s.LoadAsync(fee, fee.Key, CancellationToken.None));
			...
			await (s.DeleteAsync(fee, CancellationToken.None));
			await (s.DeleteAsync("from fee in class NHibernate.DomainModel.Fee", CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ArraysOfTimesAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.UpdateAsync(baz, CancellationToken.None)); // missing in H2.1
				await (s.FlushAsync(CancellationToken.None));
			}
		...
		public async Task ComponentsAsync()
		{
			...
			s = OpenSession();
			txn = s.BeginTransaction();
			await (s.LoadAsync(foo, foo.Key, CancellationToken.None));
			...
			// which is first_name
			Assert.AreSame(foo, (await (s.CreateQuery("from Foo foo where foo.Custom.s1 = 'one'").ListAsync(CancellationToken.None)))[0]);
			await (s.DeleteAsync(foo, CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task EnumAsync()
		{
			...
			object id = await (s.SaveAsync(foo, CancellationToken.None));
			...
			s = OpenSession();
			IList list =
				await (s.CreateQuery("select new Result(foo.String, foo.Long, foo.Integer, foo.Status) from foo in class Foo").ListAsync(CancellationToken.None));
			...
			Assert.AreEqual(1, (await (q.ListAsync(CancellationToken.None))).Count, "Enum in string - should have found OFF");

			q = s.CreateQuery("from Foo as f where f.Status = FooStatus.ON");
			Assert.AreEqual(0, (await (q.ListAsync(CancellationToken.None))).Count, "Enum in string - should not have found ON");
		...
		public async Task NoForeignKeyViolationsAsync()
		{
			...
			s = OpenSession();
			IList l = await (s.CreateQuery("from g in class NHibernate.DomainModel.Glarch where g.Next is not null").ListAsync(CancellationToken.None));
			await (s.DeleteAsync(l[0], CancellationToken.None));
			await (s.DeleteAsync(l[1], CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		...
		public async Task NewSessionLifecycleAsync()
		{
			...
			try
			{
				Foo f = (Foo) await (s.LoadAsync(typeof(Foo), fid, LockMode.Upgrade, CancellationToken.None));
				await (s.DeleteAsync(f, CancellationToken.None));
				// s.Flush();
				await (t.CommitAsync(CancellationToken.None));
			}
		...
		public async Task OrderByAsync()
		{
			...
			s = OpenSession();
			Many manyB = new Many();
			await (s.SaveAsync(manyB, CancellationToken.None));
			One oneB = new One();
			await (s.SaveAsync(oneB, CancellationToken.None));
			oneB.Value = "b";
			manyB.One = oneB;
			Many manyA = new Many();
			await (s.SaveAsync(manyA, CancellationToken.None));
			One oneA = new One();
			await (s.SaveAsync(oneA, CancellationToken.None));
			oneA.Value = "a";
			manyA.One = oneA;
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			IEnumerable enumerable =
				await (s.CreateQuery("SELECT one FROM one IN CLASS " + typeof(One).Name + " ORDER BY one.Value ASC").EnumerableAsync(CancellationToken.None));
			int count = 0;
			...
			enumerable =
				await (s.CreateQuery("SELECT many.One FROM many IN CLASS " + typeof(Many).Name +
				              " ORDER BY many.One.Value ASC, many.One.id").EnumerableAsync(CancellationToken.None));
			count = 0;
			foreach (One one in enumerable)
			{
				switch (count)
			...
			s = OpenSession();
			oneA = (One) await (s.LoadAsync(typeof(One), oneA.Key, CancellationToken.None));
			manyA = (Many) await (s.LoadAsync(typeof(Many), manyA.Key, CancellationToken.None));
			oneB = (One) await (s.LoadAsync(typeof(One), oneB.Key, CancellationToken.None));
			manyB = (Many) await (s.LoadAsync(typeof(Many), manyB.Key, CancellationToken.None));
			await (s.DeleteAsync(manyA, CancellationToken.None));
			await (s.DeleteAsync(oneA, CancellationToken.None));
			await (s.DeleteAsync(manyB, CancellationToken.None));
			await (s.DeleteAsync(oneB, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ManyToOneAsync()
		{
			...
			s = OpenSession();
			one = (One) await (s.LoadAsync(typeof(One), one.Key, CancellationToken.None));
			int countManies = one.Manies.Count;
			s.Close();

			s = OpenSession();
			many = (Many) await (s.LoadAsync(typeof(Many), many.Key, CancellationToken.None));
			Assert.IsNotNull(many.One, "many-to-one assoc");
			await (s.DeleteAsync(many.One, CancellationToken.None));
			await (s.DeleteAsync(many, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task SaveDeleteAsync()
		{
			...
			s = OpenSession();
			await (s.DeleteAsync(await (s.LoadAsync(typeof(Foo), f.Key, CancellationToken.None)), CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ProxyArrayAsync()
		{
			...
			await (s.SaveAsync(g3, CancellationToken.None));
		...
		public async Task CacheAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				await (s.SaveAsync(im, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}

			using (ISession s = OpenSession())
			{
				await (s.LoadAsync(im, im.Id, CancellationToken.None));
			}
			...
			using (ISession s = OpenSession())
			{
				...
				NHibernate.DomainModel.Immutable imFromFind =
					(NHibernate.DomainModel.Immutable) (await (s.CreateQuery("from im in class Immutable where im = ?").SetEntity(0, im).ListAsync(CancellationToken.None)))[0];
				NHibernate.DomainModel.Immutable imFromLoad = (NHibernate.DomainModel.Immutable) await (s.LoadAsync(typeof(NHibernate.DomainModel.Immutable), im.Id, CancellationToken.None));
		...
		public async Task FindLoadAsync()
		{
			...
			s = OpenSession();
			foo = (FooProxy) (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None)))[0];
			FooProxy foo2 = (FooProxy) await (s.LoadAsync(typeof(Foo), foo.Key, CancellationToken.None));
			...
			s = OpenSession();
			foo2 = (FooProxy) await (s.LoadAsync(typeof(Foo), foo.Key, CancellationToken.None));
			foo = (FooProxy) (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None)))[0];
			Assert.AreSame(foo2, foo, "find returns same object as load");
			await (s.DeleteAsync("from foo in class NHibernate.DomainModel.Foo", CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task RefreshAsync()
		{
			...
			await (s.RefreshAsync(foo, CancellationToken.None));
			Assert.AreEqual((long) -3, foo.Long);
			Assert.AreEqual(LockMode.Read, s.GetCurrentLockMode(foo));
			await (s.RefreshAsync(foo, LockMode.Upgrade, CancellationToken.None));
		...
		public async Task AutoFlushAsync()
		{
			...
			FooProxy foo = new Foo();
			await (s.SaveAsync(foo, CancellationToken.None));
			Assert.AreEqual(1, (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None))).Count,
			                "autoflush inserted row");
			foo.Char = 'X';
			Assert.AreEqual(1, (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where foo.Char='X'").ListAsync(CancellationToken.None))).Count,
			...
			s = OpenSession();
			txn = s.BeginTransaction();
			foo = (FooProxy) await (s.LoadAsync(typeof(Foo), foo.Key, CancellationToken.None));
...
(				                await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where 111 in elements(foo.Bytes)")
				                 .ListAsync(CancellationToken.None))).Count, "autoflush collection update");
				foo.Bytes[0] = 69;
				Assert.AreEqual(1,
(				                await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo where 69 in elements(foo.Bytes)")
			...
			await (s.DeleteAsync(foo, CancellationToken.None));
			Assert.AreEqual(0, (await (s.CreateQuery("from foo in class NHibernate.DomainModel.Foo").ListAsync(CancellationToken.None))).Count, "autoflush delete");
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task VetoAsync()
		{
			...
			object id = await (s.SaveAsync(v, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			await (s.UpdateAsync(v, id, CancellationToken.None));
			await (s.UpdateAsync(v, id, CancellationToken.None));
		...
		public async Task SerializableTypeAsync()
		{
			...
			s = OpenSession();
			v = (Vetoer) await (s.LoadAsync(typeof(Vetoer), id, CancellationToken.None));
		...
		public async Task AutoFlushCollectionsAsync()
		{
			...
			e = (await (s.CreateQuery("select elements(baz.StringArray) from baz in class NHibernate.DomainModel.Baz").EnumerableAsync(CancellationToken.None))).
						GetEnumerator();
			...
			e = (await (s.CreateQuery("select distinct elements(baz.StringArray) from baz in class NHibernate.DomainModel.Baz").EnumerableAsync(CancellationToken.None))
)						.GetEnumerator();
			...
			e = (await (s.CreateQuery("select foo from baz in class NHibernate.DomainModel.Baz, foo in elements(baz.FooArray)").EnumerableAsync(CancellationToken.None))
)						.GetEnumerator();
			...
			e = (await (s.CreateQuery("select elements(baz.FooArray) from baz in class NHibernate.DomainModel.Baz").EnumerableAsync(CancellationToken.None))).
						GetEnumerator();
		...
		public async Task UserProvidedConnectionAsync()
		{
			...
			ITransaction tx = s.BeginTransaction();
			await (s.CreateQuery("from foo in class NHibernate.DomainModel.Fo").ListAsync(CancellationToken.None));
			await (tx.CommitAsync(CancellationToken.None));
		...
		public async Task ComplicatedQueryAsync()
		{
			...
			IEnumerator enumer =
(				await (s.CreateQuery("from foo in class Foo where foo.Dependent.Qux.Foo.String = 'foo2'").EnumerableAsync(CancellationToken.None))).GetEnumerator();
			Assert.IsTrue(enumer.MoveNext());
			await (s.DeleteAsync(foo, CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		...
		public async Task LoadAfterDeleteAsync()
		{
			...
			try
			{
				await (s.LoadAsync(typeof(Foo), id, CancellationToken.None));
			}
			...
			try
			{
				bool proxyBoolean = ((FooProxy) await (s.LoadAsync(typeof(Foo), id, CancellationToken.None))).Boolean;
			}
			...
			await (s.SaveAsync(fo, id, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			await (s.DeleteAsync(fo, CancellationToken.None));
			err = false;

			try
			{
				await (s.LoadAsync(typeof(Fo), id, CancellationToken.None));
			}
			//catch (ObjectDeletedException ode) Changed to have same behavior of H3.2
			catch (ObjectNotFoundException ode)
			{
		...
		public async Task ObjectTypeAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				GlarchProxy g = (GlarchProxy) await (s.LoadAsync(typeof(Glarch), gid, CancellationToken.None));
				Assert.IsNotNull(g.Any);
				Assert.IsTrue(g.Any is FooProxy);
				await (s.DeleteAsync(g.Any, CancellationToken.None));
				await (s.DeleteAsync(g, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
		...
		public async Task AnyAsync()
		{
			...
			foo.Object = one;
			object fid = await (s.SaveAsync(foo, CancellationToken.None));
			object oid = one.Key;
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			IList list = await (s.CreateQuery("from Bar bar where bar.Object.id = ? and bar.Object.class = ?")
				.SetParameter(0, oid, NHibernateUtil.Int64).SetParameter(1, typeof(One).FullName, NHibernateUtil.ClassMetaType).ListAsync(CancellationToken.None));
			Assert.AreEqual(1, list.Count);
			...
			s = OpenSession();
			foo = (BarProxy) await (s.LoadAsync(typeof(Foo), fid, CancellationToken.None));
			Assert.IsNotNull(foo);
			Assert.IsTrue(foo.Object is One);
			Assert.AreEqual(oid, s.GetIdentifier(foo.Object));
			await (s.DeleteAsync(foo, CancellationToken.None));
			await (s.DeleteAsync(foo.Object, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task EmbeddedCompositeIDAsync()
		{
			...
			l.StreetNumber = 300;
			l.City = "Melbourne";
			await (s.SaveAsync(l, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			l =
				(Location)
(				await (s.CreateQuery("from l in class Location where l.CountryCode = 'AU' and l.Description='foo bar'").ListAsync(CancellationToken.None)))[0];
			...
			await (s.UpdateAsync(l, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			l.StreetNumber = 300;
			l.City = "Melbourne";
			Assert.AreSame(l, await (s.LoadAsync(typeof(Location), l, CancellationToken.None)));
			Assert.AreEqual(CultureInfo.CreateSpecificCulture("en-AU"), l.Locale);
			await (s.DeleteAsync(l, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task AutosaveChildrenAsync()
		{
			...
			Assert.AreEqual(0, (await (s.CreateQuery("from Bar bar").ListAsync(CancellationToken.None))).Count);
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task TransientOrphanDeleteAsync()
		{
			...
			while (enumer.MoveNext())
			{
				...
				await (s.DeleteAsync(cmp.Glarch, CancellationToken.None));
				cmp.Glarch = null;
			}
			...
			s = OpenSession();
			t = s.BeginTransaction();
			await (s.UpdateAsync(baz, CancellationToken.None));
			Assert.AreEqual(2, (await (s.CreateQuery("from Bar bar").ListAsync(CancellationToken.None))).Count);
			Assert.AreEqual(3, (await (s.CreateQuery("from Foo foo").ListAsync(CancellationToken.None))).Count);
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			await (s.DeleteAsync(baz, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			Assert.AreEqual(0, (await (s.CreateQuery("from Foo foo").ListAsync(CancellationToken.None))).Count);
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task ProxiesInCollectionsAsync()
		{
			...
			Bar bar2 = new Bar();
			await (s.SaveAsync(bar, CancellationToken.None));
			object bar2id = await (s.SaveAsync(bar2, CancellationToken.None));
			...
			s = OpenSession();
			BarProxy barprox = (BarProxy) await (s.LoadAsync(typeof(Bar), bid, CancellationToken.None));
			BarProxy bar2prox = (BarProxy) await (s.LoadAsync(typeof(Bar), bar2id, CancellationToken.None));
			...
			await (s.DeleteAsync("from o in class Baz", CancellationToken.None));
			await (s.DeleteAsync("from o in class Foo", CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task PSCacheAsync()
		{
			...
			using(ITransaction txn = s.BeginTransaction())
			{
				...
				Assert.AreEqual(2, (await (q.ListAsync(CancellationToken.None))).Count);

				q = s.CreateQuery("from f in class Foo");

				Assert.AreEqual(10, (await (q.ListAsync(CancellationToken.None))).Count);
				Assert.AreEqual(10, (await (q.ListAsync(CancellationToken.None))).Count);

				q.SetMaxResults(3);
				q.SetFirstResult(3);

				Assert.AreEqual(3, (await (q.ListAsync(CancellationToken.None))).Count);
			...
			using (ITransaction txn = s.BeginTransaction())
			{
				...
				q.SetMaxResults(5);
				Assert.AreEqual(5, (await (q.ListAsync(CancellationToken.None))).Count);

				await (s.DeleteAsync("from f in class Foo", CancellationToken.None));
				await (txn.CommitAsync(CancellationToken.None));
			}
		}
		...
		public async Task FormulaAsync()
		{
			...
			s = OpenSession();
			foo = (Foo) (await (s.CreateQuery("from Foo as f where f.id = ?").SetParameter(0, id, NHibernateUtil.String).ListAsync(CancellationToken.None)))[0];
		...
		public async Task AddAllAsync()
		{
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(foo2, CancellationToken.None));
				Foo foo3 = new Foo();
				await (s.SaveAsync(foo3, CancellationToken.None));
				Baz baz = new Baz();
		...
		public async Task CopyAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(persistentBaz, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			...
			using (ISession s = OpenSession())
			{
				Baz persistentBaz = await (s.GetAsync(typeof(Baz), baz.Code, CancellationToken.None)) as Baz;
				Baz copiedBaz = s.Merge(baz);
				Assert.AreSame(persistentBaz, copiedBaz);

				await (s.DeleteAsync(persistentBaz, CancellationToken.None));
				await (s.FlushAsync(CancellationToken.None));
			}
		}
