...
namespace NHibernate.Test.Legacy
{
	...
	public partial class SQLLoaderTestAsync : TestCaseAsync
	{
		protected override IList Mappings
		{
			get
			{
				return new string[]{"ABC.hbm.xml", "Category.hbm.xml", "Simple.hbm.xml", "Fo.hbm.xml", "SingleSeveral.hbm.xml", "Componentizable.hbm.xml"};
			}
		}
		...
		public async Task FindBySQLStarAsync()
		{
			...
			//B b = new B();
			//session.Save( b );
			await (session.CreateSQLQuery("select {category.*} from Category {category}").AddEntity("category", typeof (Category)).ListAsync());
			await (session.CreateSQLQuery("select {simple.*} from Simple {simple}").AddEntity("simple", typeof (Simple)).ListAsync());
			await (session.CreateSQLQuery("select {a.*} from A {a}").AddEntity("a", typeof (A)).ListAsync());
		...
		public async Task FindBySQLAssociatedObjectAsync()
		{
			...
			s.Close();
			s = OpenSession();
			IList list = await (s.CreateSQLQuery("select {category.*} from Category {category}").AddEntity("category", typeof (Category)).ListAsync());
		...
		public async Task FindBySQLMultipleObjectAsync()
		{
			...
			{
				IList list = await (s.CreateSQLQuery("select {category.*}, {assignable.*} from Category {category}, \"assign able\" {assignable}").AddEntity("category", typeof (Category)).AddEntity("assignable", typeof (Assignable)).ListAsync());
