	...
	public class FumTestAsync : TestCase
	{
		...
		public async Task CriteriaCollectionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				await (s.SaveAsync(fum, CancellationToken.None));
		...
		public async Task CriteriaAsync()
		{
			...
			using (ITransaction txn = s.BeginTransaction())
			{
				...
				await (s.SaveAsync(fr, CancellationToken.None));
				await (s.SaveAsync(fr2, CancellationToken.None));
				await (s.SaveAsync(fum.Fo, CancellationToken.None));
				await (s.SaveAsync(fum, CancellationToken.None));
				...
				IList list = await (baseCriteria.ListAsync(CancellationToken.None));
				...
				IDictionary map = (IDictionary) await (baseCriteria.UniqueResultAsync(CancellationToken.None));
				...
				map = (IDictionary) (await (baseCriteria.ListAsync(CancellationToken.None)))[0];
			...
			using (ITransaction txn = s.BeginTransaction())
			{
				...
				baseCriteria.CreateCriteria("Friends")
					.Add(Expression.Like("FumString", "g%"));
				Fum fum = (Fum) (await (baseCriteria.ListAsync(CancellationToken.None)))[0];
				Assert.AreEqual(2, fum.Friends.Count);
				await (s.DeleteAsync(fum, CancellationToken.None));
				await (s.DeleteAsync(fum.Fo, CancellationToken.None));

				foreach (object friend in fum.Friends)
				{
					await (s.DeleteAsync(friend, CancellationToken.None));
				}
				await (txn.CommitAsync(CancellationToken.None));
			}
		}
		...
		public async Task ListIdentifiersAsync()
		{
			...
			// different FlushMode
			IList list =
				await (s.CreateQuery("select fum.Id from fum in class NHibernate.DomainModel.Fum where not fum.FumString = 'FRIEND'").ListAsync(CancellationToken.None));
...
(				await (s.CreateQuery("select fum.Id from fum in class NHibernate.DomainModel.Fum where not fum.FumString='FRIEND'").
			...
			await (s.DeleteAsync(await (s.LoadAsync(typeof(Fum), list[0], CancellationToken.None)), CancellationToken.None));
			await (s.DeleteAsync(await (s.LoadAsync(typeof(Fum), list[1], CancellationToken.None)), CancellationToken.None));
			await (txn.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CompositeIDAsync()
		{
			...
			Assert.AreSame(fum, await (s.LoadAsync(typeof(Fum), FumKey("fum"), LockMode.Upgrade, CancellationToken.None)));
			//s.Flush();
			...
			fum = (Fum) await (s.LoadAsync(typeof(Fum), FumKey("fum"), LockMode.Upgrade, CancellationToken.None));
			Assert.IsNotNull(fum, "Load by composite key");

			Fum fum2 = new Fum(FumKey("fi"));
			fum2.FumString = "fee fo fi";
			fum.Fo = fum2;
			await (s.SaveAsync(fum2, CancellationToken.None));

			IList list = await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where not fum.FumString='FRIEND'").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count, "Find a List of Composite Keyed objects");

			IList list2 =
				await (s.CreateQuery("select fum from fum in class NHibernate.DomainModel.Fum where fum.FumString='fee fi fo'").ListAsync(CancellationToken.None));
			...
			t = s.BeginTransaction();
			IEnumerator enumerator =
(				await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where not fum.FumString='FRIEND'").EnumerableAsync(CancellationToken.None))).
					GetEnumerator();
			int i = 0;
			while (enumerator.MoveNext())
			{
		...
		public async Task CompositeIDOneToOneAsync()
		{
			...
			Fumm fumm = new Fumm();
			fumm.Fum = fum;
			await (s.SaveAsync(fumm, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			fumm = (Fumm) await (s.LoadAsync(typeof(Fumm), FumKey("fum"), CancellationToken.None));
			//s.delete(fumm.Fum); commented out in h2.0.3
			await (s.DeleteAsync(fumm, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CompositeIDQueryAsync()
		{
			...
			await (s.SaveAsync(fi, CancellationToken.None));
			Fum fo = new Fum(FumKey("fo", true));
			fo.FumString = "fo";
			await (s.SaveAsync(fo, CancellationToken.None));
			...
			IList vList = await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where fum.Id.String='fo'").ListAsync(CancellationToken.None));
			...
			vList =
				await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where fum.Id.Short = ?").SetInt16(0, fiShort).ListAsync(CancellationToken.None));
			...
			vList =
				await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where fum.Id.Date <= ? and not fum.FumString='FRIEND'").
					SetDateTime(0, DateTime.Now).ListAsync(CancellationToken.None));
...
(				await (s.CreateQuery("select fum.Id.Short, fum.Id.Date, fum.Id.String from fum in class NHibernate.DomainModel.Fum").
					EnumerableAsync(CancellationToken.None))).GetEnumerator().MoveNext());
			Assert.IsTrue(
(				await (s.CreateQuery("select fum.Id from fum in class NHibernate.DomainModel.Fum").EnumerableAsync(CancellationToken.None))).GetEnumerator().MoveNext());
			...
			IEnumerator enumer = (await (qu.EnumerableAsync(CancellationToken.None))).GetEnumerator();
			int j = 0;
			...
			fum = (Fum) await (s.LoadAsync(typeof(Fum), fum.Id, CancellationToken.None));
			await ((await (s.CreateFilterAsync(fum.QuxArray, "where this.Foo is null", CancellationToken.None))).ListAsync(CancellationToken.None));
(			await (s.CreateFilterAsync(fum.QuxArray, "where this.Foo.id = ?", CancellationToken.None))).SetString(0, "fooid");
			IQuery f = await (s.CreateFilterAsync(fum.QuxArray, "where this.Foo.id = :fooId", CancellationToken.None));
			f.SetString("fooId", "abc");
			Assert.IsFalse((await (f.EnumerableAsync(CancellationToken.None))).GetEnumerator().MoveNext());
...
(				await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where not fum.FumString='FRIEND'").EnumerableAsync(CancellationToken.None))).
			...
			await (s.CreateQuery("from Fumm f1 inner join f1.Fum f2").ListAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CompositeIDCollectionsAsync()
		{
			...
			fum2.FumString = "fee fo fi";
			await (s.SaveAsync(fum1, CancellationToken.None));
			await (s.SaveAsync(fum2, CancellationToken.None));
			Qux q = new Qux();
			...
			enumer.MoveNext();
			await (s.DeleteAsync((Fum) enumer.Current, CancellationToken.None));
			enumer.MoveNext();
			await (s.DeleteAsync((Fum) enumer.Current, CancellationToken.None));
		...
		public async Task DeleteOwnerAsync()
		{
			...
			s = OpenSession();
			ITransaction t = s.BeginTransaction();
			q = (Qux) await (s.LoadAsync(typeof(Qux), q.Key, LockMode.Upgrade, CancellationToken.None));
			await (s.LockAsync(q, LockMode.Upgrade, CancellationToken.None));
			await (s.DeleteAsync(q, CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			t = s.BeginTransaction();
			var list = await (s.CreateQuery("from fum in class NHibernate.DomainModel.Fum where not fum.FumString='FRIEND'").ListAsync(CancellationToken.None));
			Assert.AreEqual(2, list.Count, "deleted owner");
			await (s.LockAsync(list[0], LockMode.Upgrade, CancellationToken.None));
			await (s.LockAsync(list[1], LockMode.Upgrade, CancellationToken.None));
		...
		public async Task CompositeIDsAsync()
		{
			ISession s = OpenSession();
			Fo fo = Fo.NewFo();
			await (s.SaveAsync(fo, FumKey("an instance of fo"), CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			fo = (Fo) await (s.LoadAsync(typeof(Fo), FumKey("an instance of fo"), CancellationToken.None));
			fo.X = 5;
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
			...
			IEnumerator enumer =
(				await (s.CreateQuery("from fo in class NHibernate.DomainModel.Fo where fo.id.String like 'an instance of fo'").EnumerableAsync(CancellationToken.None))
)					.GetEnumerator();
			...
			try
			{
				await (s.SaveAsync(Fo.NewFo(), CancellationToken.None));
		...
		public async Task KeyManyToOneAsync()
		{
			...
			d.Id = did;
			d.Bubu = "bubu";
			await (s.SaveAsync(sup, CancellationToken.None));
			...
			s = OpenSession();
			d = (Outer) await (s.LoadAsync(typeof(Outer), did, CancellationToken.None));
			Assert.AreEqual("dudu", d.Id.Master.Id.Sup.Dudu);
			await (s.DeleteAsync(d, CancellationToken.None));
			await (s.DeleteAsync(d.Id.Master, CancellationToken.None));
			await (s.SaveAsync(d.Id.Master, CancellationToken.None));
			await (s.SaveAsync(d, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();

			s = OpenSession();
			d = (Outer) (await (s.CreateQuery("from Outer o where o.id.DetailId=?").SetString(0, d.Id.DetailId).ListAsync(CancellationToken.None)))[0];
			await (s.CreateQuery("from Outer o where o.Id.Master.Id.Sup.Dudu is not null").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Outer o where o.Id.Master.Bla = ''").ListAsync(CancellationToken.None));
			await (s.CreateQuery("from Outer o where o.Id.Master.Id.One = ''").ListAsync(CancellationToken.None));
			await (s.DeleteAsync(d, CancellationToken.None));
			await (s.DeleteAsync(d.Id.Master, CancellationToken.None));
			await (s.DeleteAsync(d.Id.Master.Id.Sup, CancellationToken.None));
			await (s.FlushAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task CompositeKeyPathExpressionsAsync()
		{
			using (ISession s = OpenSession())
			{
				await (s.CreateQuery("select fum1.Fo from fum1 in class Fum where fum1.Fo.FumString is not null").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from fum1 in class Fum where fum1.Fo.FumString is not null order by fum1.Fo.FumString").ListAsync(CancellationToken.None));
				if (Dialect.SupportsSubSelects)
				{
					await (s.CreateQuery("from fum1 in class Fum where exists elements(fum1.Friends)").ListAsync(CancellationToken.None));
					await (s.CreateQuery("from fum1 in class Fum where size(fum1.Friends) = 0").ListAsync(CancellationToken.None));
				}
				await (s.CreateQuery("select elements(fum1.Friends) from fum1 in class Fum").ListAsync(CancellationToken.None));
				await (s.CreateQuery("from fum1 in class Fum, fr in elements( fum1.Friends )").ListAsync(CancellationToken.None));
			}
		}
		...
		public async Task UnflushedSessionSerializationAsync()
		{
			...
			using (ISession s = sessions.OpenSession())
			{
				...
				await (s.SaveAsync(simple, 10L, CancellationToken.None));
			...
			using (ISession s = s2)
			{
				...
				Simple simple = (Simple) await (s.LoadAsync(typeof(Simple), 10L, CancellationToken.None));
				other = new Simple();
				other.Init();
				await (s.SaveAsync(other, 11L, CancellationToken.None));
			...
			{
				s.FlushMode = FlushMode.Never;
				Simple simple = (Simple) await (s.GetAsync(typeof(Simple), 10L, CancellationToken.None));
