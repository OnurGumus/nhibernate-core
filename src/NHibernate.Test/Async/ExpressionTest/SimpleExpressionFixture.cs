...
namespace NHibernate.Test.ExpressionTest
{
	...
	public partial class SimpleExpressionFixtureAsync : BaseExpressionFixture
	{
		[Test]
		...
		[Test]
		public async Task MisspelledPropertyWithNormalizedEntityPersisterAsync()
		{
			using (ISession session = factory.OpenSession())
			{
				CreateObjects(typeof (Multi), session);
				ICriterion expression = Expression.Eq("MisspelledProperty", DateTime.Now);
				Assert.ThrowsAsync<QueryException>(async () => await (expression.ToSqlStringAsync(criteria, criteriaQuery, new CollectionHelper.EmptyMapClass<string, IFilter>())));
			}
		}
	}
}
