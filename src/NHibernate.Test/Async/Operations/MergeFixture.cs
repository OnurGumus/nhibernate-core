...
namespace NHibernate.Test.Operations
{
	...
	public partial class MergeFixture : AbstractOperationTestCase
	{
		...
		public async Task MergeManyToManyWithCollectionDeferenceAsync()
		{
			...
			//   3) attempt the merge
			var competition2 = (Competition)await (s.MergeAsync(competition));
			await (tx.CommitAsync());
			s.Close();
		...
		public async Task NoExtraUpdatesOnMergeVersionedAsync()
		{
			...
			s = OpenSession();
			s.BeginTransaction();
			var mergedEntity = (VersionedEntity)await (s.MergeAsync(entity));
			await (s.Transaction.CommitAsync());
			s.Close();
			AssertUpdateCount(0);
			AssertInsertCount(0);
		...
		public async Task NoExtraUpdatesOnMergeVersionedWithCollectionAsync()
		{
			...
			s = OpenSession();
			s.BeginTransaction();
			var mergedParent = (VersionedEntity)await (s.MergeAsync(parent));
			await (s.Transaction.CommitAsync());
			s.Close();
			AssertUpdateCount(0);
			AssertInsertCount(0);
