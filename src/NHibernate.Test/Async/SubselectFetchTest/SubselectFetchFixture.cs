...
namespace NHibernate.Test.SubselectFetchTest
{
	...
	public partial class SubselectFetchFixture : TestCase
	{
		...
		public async Task SubselectFetchHqlAsync()
		{
			...
			IList parents = s.CreateQuery("from Parent where name between 'bar' and 'foo' order by name desc").List();
			p = (Parent)parents[0];
			q = (Parent)parents[1];
		...
		public async Task SubselectFetchNamedParamAsync()
		{
			...
			t = s.BeginTransaction();
			sessions.Statistics.Clear();
			IList parents = s.CreateQuery("from Parent where name between :bar and :foo order by name desc").SetParameter("bar", "bar").SetParameter("foo", "foo").List();
			p = (Parent)parents[0];
			q = (Parent)parents[1];
			Assert.IsFalse(NHibernateUtil.IsInitialized(p.Children));
			Assert.IsFalse(NHibernateUtil.IsInitialized(q.Children));
		...
		public async Task SubselectFetchPosParamAsync()
		{
			...
			t = s.BeginTransaction();
			sessions.Statistics.Clear();
			IList parents = s.CreateQuery("from Parent where name between ? and ? order by name desc").SetParameter(0, "bar").SetParameter(1, "foo").List();
			p = (Parent)parents[0];
			q = (Parent)parents[1];
			Assert.IsFalse(NHibernateUtil.IsInitialized(p.Children));
			Assert.IsFalse(NHibernateUtil.IsInitialized(q.Children));
		...
		public async Task SubselectFetchWithLimitAsync()
		{
			...
			t = s.BeginTransaction();
			sessions.Statistics.Clear();
			IList parents = s.CreateQuery("from Parent order by name desc").SetMaxResults(2).List();
			p = (Parent)parents[0];
			q = (Parent)parents[1];
			Assert.IsFalse(NHibernateUtil.IsInitialized(p.Children));
			Assert.IsFalse(NHibernateUtil.IsInitialized(p.MoreChildren));
		...
		public async Task ManyToManyCriteriaJoinAsync()
		{
			...
			s = OpenSession();
			t = s.BeginTransaction();
			s.CreateCriteria(typeof (Parent)).AddOrder(Order.Desc("Name"))// H3 has this after CreateCriteria("Friends"), but it's not yet supported in NH
			.CreateCriteria("MoreChildren").CreateCriteria("Friends").List();
			IList parents = s.CreateCriteria(typeof (Parent)).SetFetchMode("MoreChildren", FetchMode.Join).SetFetchMode("MoreChildren.Friends", FetchMode.Join).AddOrder(Order.Desc("Name")).List();
		...
		public async Task SubselectFetchCriteriaAsync()
		{
			...
			t = s.BeginTransaction();
			sessions.Statistics.Clear();
			IList parents = s.CreateCriteria(typeof (Parent)).Add(Expression.Between("Name", "bar", "foo")).AddOrder(Order.Desc("Name")).List();
			p = (Parent)parents[0];
			q = (Parent)parents[1];
			Assert.IsFalse(NHibernateUtil.IsInitialized(p.Children));
			Assert.IsFalse(NHibernateUtil.IsInitialized(q.Children));
