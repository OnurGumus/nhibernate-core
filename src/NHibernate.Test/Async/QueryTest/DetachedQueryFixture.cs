...
namespace NHibernate.Test.QueryTest
{
	...
	public partial class DetachedQueryFixtureAsync : TestCaseAsync
	{
		...
		protected override IList Mappings
		{
			get
			{
				return new string[]{"QueryTest.DetachedQueryTest.hbm.xml"};
			}
		}
		...
		[Test]
		public async Task PropertiesSetAsync()
		{
			TestDetachedQuery tdq = new TestDetachedQuery();
			tdq.SetMaxResults(10).SetFirstResult(5).SetCacheable(true).SetReadOnly(true).SetTimeout(444).SetFlushMode(FlushMode.Auto).SetCacheRegion("A_REGION").SetResultTransformer(new AliasToBeanResultTransformer(typeof (NoFoo))).SetIgnoreUknownNamedParameters(true).SetComment(MyComment);
			...
			await (tdq.SetEntityAsync(9, new Foo("Fulano", "De Tal")));
		...
		public async Task CopyToTestAsync()
		{
			...
			origin.SetMaxResults(10).SetFirstResult(5).SetCacheable(true).SetReadOnly(true).SetTimeout(444).SetFlushMode(FlushMode.Auto).SetCacheRegion("A_REGION").SetResultTransformer(new AliasToBeanResultTransformer(typeof (NoFoo)));
		...
		public async Task ResultTransformerAsync()
		{
			...
			dq.SetString("p1", "%1_").SetResultTransformer(new AliasToBeanResultTransformer(typeof (NoFoo)));
			using (ISession s = OpenSession())
			{
				IQuery q = dq.GetExecutableQuery(s);
		...
		private class TestDetachedQuery : AbstractDetachedQuery
		{
			...
			public bool ShouldIgnoredUnknownNamedParameters
			{
				get
				{
					return shouldIgnoredUnknownNamedParameters;
				}
			}
