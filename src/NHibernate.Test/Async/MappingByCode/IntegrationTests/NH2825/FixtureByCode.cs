...
namespace NHibernate.Test.MappingByCode.IntegrationTests.NH2825
{
	...
	public partial class FixtureByCodeAsync : FixtureAsync
	{
		...
		private HbmMapping GetMappings()
		{
			...
			mapper.Class<Parent>(rc =>
			{
				...
				rc.Bag(p => p.Children, m =>
				{
					m.Key(km =>
					{
						km.Column(cm => cm.Name("ParentParentCode"));
						km.PropertyRef(pg => pg.ParentCode);
					}
			...
			mapper.Class<Child>(rc =>
			{
				rc.Id(p => p.Id);
				rc.Property(p => p.Name);
				rc.ManyToOne<Parent>(p => p.Parent, m =>
				{
					m.Column("ParentParentCode");
					m.PropertyRef("ParentCode");
				}
