...
namespace NHibernate.Test.Hql.Ast
{
	...
	public class BulkManipulationAsync : BaseFixture
	{
		...
		public void DeleteNonExistentEntityAsync()
		{
			...
			{
				Assert.ThrowsAsync<QuerySyntaxException>(() => s.CreateQuery("delete NonExistentEntity").ExecuteUpdateAsync(CancellationToken.None));
			}
		}
		...
		public void UpdateNonExistentEntityAsync()
		{
			using (ISession s = OpenSession())
			{
				Assert.ThrowsAsync<QuerySyntaxException>(
					() => s.CreateQuery("update NonExistentEntity e set e.someProp = ?").ExecuteUpdateAsync(CancellationToken.None));
			}
		}
		...
		public async Task SimpleInsertAsync()
		{
			...
			await (s.CreateQuery("insert into Pickup (id, Vin, Owner) select id, Vin, Owner from Car").ExecuteUpdateAsync(CancellationToken.None));

			await (t.CommitAsync(CancellationToken.None));
			t = s.BeginTransaction();

			await (s.CreateQuery("delete Vehicle").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task InsertWithMismatchedTypesAsync()
		{
			...
			Assert.ThrowsAsync<QueryException>(
			  () => s.CreateQuery("insert into Pickup (Owner, Vin, id) select id, Vin, Owner from Car").ExecuteUpdateAsync(CancellationToken.None),
		...
		public async Task InsertIntoSuperclassPropertiesFailsAsync()
		{
			...
			Assert.ThrowsAsync<QueryException>(
				() => s.CreateQuery("insert into Human (id, bodyWeight) select id, bodyWeight from Lizard").ExecuteUpdateAsync(CancellationToken.None),
				"superclass prop insertion did not error");

			await (t.CommitAsync(CancellationToken.None));
			t = s.BeginTransaction();

			await (s.CreateQuery("delete Animal where mother is not null").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete Animal where father is not null").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete Animal").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task InsertAcrossMappedJoinFailsAsync()
		{
			...
			Assert.ThrowsAsync<QueryException>(
				() => s.CreateQuery("insert into Joiner (name, joinedName) select vin, owner from Car").ExecuteUpdateAsync(CancellationToken.None),
				"mapped-join insertion did not error");

			await (t.CommitAsync(CancellationToken.None));
			t = s.BeginTransaction();

			await (s.CreateQuery("delete Joiner").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete Vehicle").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task InsertWithGeneratedVersionAndIdAsync()
		{
			...
			await (s.CreateQuery("select id, name, version from IntegerVersioned").ListAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			var created =
				(IntegerVersioned)
				await (s.CreateQuery("from IntegerVersioned where id <> :initialId").SetInt64("initialId", initialId).UniqueResultAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			t = s.BeginTransaction();
			await (s.CreateQuery("delete IntegerVersioned").ExecuteUpdateAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task InsertWithGeneratedTimestampVersionAsync()
		{
			...
			await (s.SaveAsync(entity, CancellationToken.None));
			await (s.CreateQuery("select id, name, version from TimestampVersioned").ListAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			var created =
				(TimestampVersioned)
				await (s.CreateQuery("from TimestampVersioned where id <> :initialId").SetInt64("initialId", initialId).UniqueResultAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			t = s.BeginTransaction();
			await (s.CreateQuery("delete TimestampVersioned").ExecuteUpdateAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task UpdateWithWhereExistsSubqueryAsync()
		{
			...
			await (s.SaveAsync(doll, CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
			...
			string updateQryString = "update Human h " + "set h.description = 'updated' " + "where exists ("
									 ...
									 + ")";
			int count = await (s.CreateQuery(updateQryString).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1));
			await (s.DeleteAsync(doll, CancellationToken.None));
			...
			count = await (s.CreateQuery(updateQryString).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1));
			// many-to-many test
			...
			mtm.MoveNext();
			await (s.DeleteAsync(mtm.Current, CancellationToken.None));
		...
		public async Task IncrementCounterVersionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				using (ITransaction t = s.BeginTransaction())
				{
					entity = await (s.GetAsync<IntegerVersioned>(entity.Id, CancellationToken.None));
					await (s.DeleteAsync(entity, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
		...
		public async Task IncrementTimestampVersionAsync()
		{
			...
			using (ISession s = OpenSession())
			{
				...
				using (ITransaction t = s.BeginTransaction())
				{
					entity = await (s.LoadAsync<TimestampVersioned>(entity.Id, CancellationToken.None));
					await (s.DeleteAsync(entity, CancellationToken.None));
					await (t.CommitAsync(CancellationToken.None));
				}
			}
		...
		public async Task UpdateOnComponentAsync()
		{
			...
			await (s.SaveAsync(human, CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			...
			int count =
				await (s.CreateQuery("update Human set name.first = :correction where id = :id").SetString("correction", correctName).
					SetInt64("id", human.Id).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "incorrect update count");
			await (t.CommitAsync(CancellationToken.None));

			t = s.BeginTransaction();
			await (s.RefreshAsync(human, CancellationToken.None));

			Assert.That(human.Name.First, Is.EqualTo(correctName), "Update did not execute properly");

			await (s.CreateQuery("delete Human").ExecuteUpdateAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
		...
		public async Task UpdateOnManyToOneAsync()
		{
			...
			await (s.CreateQuery("update Animal a set a.mother = null where a.id = 2").ExecuteUpdateAsync(CancellationToken.None));
			if (Dialect.SupportsSubqueryOnMutatingTable)
			{
				await (s.CreateQuery("update Animal a set a.mother = (from Animal where id = 1) where a.id = 2").ExecuteUpdateAsync(CancellationToken.None));
			}
		...
		public async Task UpdateOnImplicitJoinFailsAsync()
		{
			...
			await (s.SaveAsync(human, CancellationToken.None));
			await (s.SaveAsync(mother, CancellationToken.None));
			...
			var e =
				Assert.ThrowsAsync<QueryException>(
					() => s.CreateQuery("update Human set mother.name.initial = :initial").SetString("initial", "F").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(e.Message, Does.StartWith("Implied join paths are not assignable in update"));

			await (s.CreateQuery("delete Human where mother is not null").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete Human").ExecuteUpdateAsync(CancellationToken.None));
			await (t.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task UpdateOnDiscriminatorSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("update PettingZoo set name = name").ExecuteUpdateAsync(CancellationToken.None));
			...
			count = await (s.CreateQuery("update Zoo as z set z.name = z.name").ExecuteUpdateAsync(CancellationToken.None));
			...
			count = await (s.CreateQuery("update Zoo as z set name = name where id = :id").SetInt64("id", data.Zoo.Id).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass update count");
		...
		public async Task UpdateOnAnimalAsync()
		{
			...
			var tadpole = await (s.LoadAsync<Animal>(data.Polliwog.Id, CancellationToken.None));
			...
			if (Dialect.SupportsSubqueryOnMutatingTable)
			{
				await (s.CreateQuery("update Animal set bodyWeight = ( select max(bodyWeight) from Animal )").ExecuteUpdateAsync(CancellationToken.None));
			}
		...
		public async Task UpdateMultiplePropertyOnAnimalAsync()
		{
			...
			{
				var tadpole = await (s.GetAsync<Animal>(data.Polliwog.Id, CancellationToken.None));
		...
		public async Task UpdateOnMammalAsync()
		{
			...
			int count = await (s.CreateQuery("update Mammal set description = description").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(2), "incorrect update count against 'middle' of joined-subclass hierarchy");

			count = await (s.CreateQuery("update Mammal set bodyWeight = 25").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(2), "incorrect update count against 'middle' of joined-subclass hierarchy");

			if (Dialect.SupportsSubqueryOnMutatingTable)
			{
				count = await (s.CreateQuery("update Mammal set bodyWeight = ( select max(bodyWeight) from Animal )").ExecuteUpdateAsync(CancellationToken.None));
				Assert.That(count, Is.EqualTo(2), "incorrect update count against 'middle' of joined-subclass hierarchy");
			}
		...
		public async Task UpdateSetNullUnionSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("update Vehicle set Owner = 'Steve'").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(4), "incorrect restricted update count");
			count = await (s.CreateQuery("update Vehicle set Owner = null where Owner = 'Steve'").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(4), "incorrect restricted update count");

			count = await (s.CreateQuery("delete Vehicle where Owner is null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(4), "incorrect restricted update count");
		...
		public void WrongPropertyNameThrowQueryExceptionAsync()
		{
			using (ISession s = OpenSession())
			{
				var e = Assert.ThrowsAsync<QueryException>(() => s.CreateQuery("update Vehicle set owner = null where owner = 'Steve'").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task UpdateSetNullOnDiscriminatorSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("update PettingZoo set address.city = null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");
			count = await (s.CreateQuery("delete Zoo where address.city is null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");

			count = await (s.CreateQuery("update Zoo set address.city = null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");
			count = await (s.CreateQuery("delete Zoo where address.city is null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");
		...
		public async Task UpdateSetNullOnJoinedSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("update Mammal set bodyWeight = null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(2), "Incorrect deletion count on joined subclass");

			count = await (s.CreateQuery("delete Animal where bodyWeight = null").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(2), "Incorrect deletion count on joined subclass");
		...
		public async Task DeleteWithSubqueryAsync()
		{
			...
			await (s.SaveAsync(owner2, CancellationToken.None));
			var owner3 = new SimpleEntityWithAssociation {Name = "myEntity-3"};
			await (s.SaveAsync(owner3, CancellationToken.None));
			await (s.Transaction.CommitAsync(CancellationToken.None));
			s.Close();
			...
			s = OpenSession();
			s.BeginTransaction();
			await (s.CreateQuery("delete SimpleAssociatedEntity").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete SimpleEntityWithAssociation").ExecuteUpdateAsync(CancellationToken.None));
			await (s.Transaction.CommitAsync(CancellationToken.None));
			s.Close();
		}
		...
		public async Task SimpleDeleteOnAnimalAsync()
		{
			...
			int count =
				await (s.CreateQuery("delete from Animal as a where a.id = :id").SetInt64("id", data.Polliwog.Id).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect delete count");

			count = await (s.CreateQuery("delete Animal where id = :id").SetInt64("id", data.Catepillar.Id).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect delete count");

			// HHH-873...
			if (Dialect.SupportsSubqueryOnMutatingTable)
			{
				count = await (s.CreateQuery("delete from User u where u not in (select u from User u)").ExecuteUpdateAsync(CancellationToken.None));
				Assert.That(count, Is.EqualTo(0));
			}

			count = await (s.CreateQuery("delete Animal a").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(4), "Incorrect delete count");

			IList list = await (s.CreateQuery("select a from Animal as a").ListAsync(CancellationToken.None));
		...
		public async Task DeleteOnDiscriminatorSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("delete PettingZoo").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");

			count = await (s.CreateQuery("delete Zoo").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect discrim subclass delete count");
		...
		public async Task DeleteOnJoinedSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("delete Mammal where bodyWeight > 150").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect deletion count on joined subclass");

			count = await (s.CreateQuery("delete Mammal").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "Incorrect deletion count on joined subclass");

			count = await (s.CreateQuery("delete SubMulti").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task DeleteOnMappedJoinAsync()
		{
			...
			int count =
				await (s.CreateQuery("delete Joiner where joinedName = :joinedName").SetString("joinedName", "joined-name").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task DeleteUnionSubclassAbstractRootAsync()
		{
			...
			int count = await (s.CreateQuery("delete Vehicle where Owner = :owner").SetString("owner", "Steve").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "incorrect restricted update count");

			count = await (s.CreateQuery("delete Vehicle").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task DeleteUnionSubclassConcreteSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("delete Truck where Owner = :owner").SetString("owner", "Steve").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "incorrect restricted update count");

			count = await (s.CreateQuery("delete Truck").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task DeleteUnionSubclassLeafSubclassAsync()
		{
			...
			int count = await (s.CreateQuery("delete Car where Owner = :owner").SetString("owner", "Kirsten").ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1), "incorrect restricted update count");

			count = await (s.CreateQuery("delete Car").ExecuteUpdateAsync(CancellationToken.None));
		...
		public async Task DeleteRestrictedOnManyToOneAsync()
		{
			...
			int count = await (s.CreateQuery("delete Animal where mother = :mother").SetEntity("mother", data.Butterfly).ExecuteUpdateAsync(CancellationToken.None));
			Assert.That(count, Is.EqualTo(1));
		...
		public async Task DeleteSyntaxWithCompositeIdAsync()
		{
			...
			await (s.CreateQuery("delete EntityWithCrazyCompositeKey where Id.Id = 1 and Id.OtherId = 2").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete from EntityWithCrazyCompositeKey where Id.Id = 1 and Id.OtherId = 2").ExecuteUpdateAsync(CancellationToken.None));
			await (s.CreateQuery("delete from EntityWithCrazyCompositeKey e where e.Id.Id = 1 and e.Id.OtherId = 2").ExecuteUpdateAsync(CancellationToken.None));
