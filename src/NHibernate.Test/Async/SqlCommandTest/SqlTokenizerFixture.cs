...
namespace NHibernate.Test.SqlCommandTest
{
	...
	public partial class SqlTokenizerFixtureAsync
	{
		[Test]
		public void TokenizeSimpleSelectStatement()
		{
			VerifyTokenizer("SELECT * FROM some_table WHERE key = ? ORDER BY some_field DESC", Text("SELECT"), Whitespace(" "), Text("*"), Whitespace(" "), Text("FROM"), Whitespace(" "), Text("some_table"), Whitespace(" "), Text("WHERE"), Whitespace(" "), Text("key"), Whitespace(" "), Text("="), Whitespace(" "), Parameter(), Whitespace(" "), Text("ORDER"), Whitespace(" "), Text("BY"), Whitespace(" "), Text("some_field"), Whitespace(" "), Text("DESC"));
		}
		...
		public void TokenizeLineComments()
		{
			...
			VerifyTokenizer("-- Any comment will do", Comment("-- Any comment will do"));
			VerifyTokenizer("-- Two comments\n--will do too", Comment("-- Two comments"), Whitespace("\n"), Comment("--will do too"));
		}
		...
		public void TokenizeBrackets()
		{
			...
			VerifyTokenizer("SELECT (SELECT COUNT(*) FROM table), ?", Text("SELECT"), Whitespace(" "), BracketOpen(), Text("SELECT"), Whitespace(" "), Text("COUNT"), BracketOpen(), Text("*"), BracketClose(), Whitespace(" "), Text("FROM"), Whitespace(" "), Text("table"), BracketClose(), Comma(), Whitespace(" "), Parameter());
		}
		...
		public void TokenizeParameters()
		{
			...
			VerifyTokenizer("EXEC InsertSomething ?, ?", Text("EXEC"), Whitespace(" "), Text("InsertSomething"), Whitespace(" "), Parameter(), Comma(), Whitespace(" "), Parameter());
		}
		...
		private static void VerifyTokenizer(string sql, params ExpectedToken[] expectedTokens)
		{
			...
			{IgnoreComments = false, IgnoreWhitespace = false})
			{
				...
				{
					Assert.Fail("Tokenizer returns more than expected '{0}' tokens. \nSQL: {1}\nLast Token: {2}({3})", expectedTokens.Length, sql, token.TokenType, token.Value);
				}
