...
namespace NHibernate.Test.SqlCommandTest
{
	...
	public partial class SqlStringFixtureAsync
	{
		//[Test]
		...
		public void Parse()
		{
			...
			SqlString expectedSql = new SqlString(new object[]{"select col from table where col = ", Parameter.Placeholder, " or col1 = 'a?b' and col2 = ", Parameter.Placeholder, " and col3 = 'x' and col4 = ", Parameter.Placeholder});
		...
		{
			SqlString sql = SqlString.Parse("select (select foo from bar where foo=col order by foo) from table where col = ? order by col");
			Assert.AreEqual(" from table where col = ? ", sql.GetSubselectString().ToString());
		}
		...
		{
			SqlString sql = SqlString.Parse("select (select foo from bar where foo=col order by foo) from table where col = (select yadda from blah where yadda=x order by yadda) order by col");
			Assert.AreEqual(" from table where col = (select yadda from blah where yadda=x order by yadda) ", sql.GetSubselectString().ToString());
		}

		[Test]
		public void GetSubselectStringWithFormulaPropertyWithSubselect()
		{
			SqlString sql = SqlString.Parse("select (select (select blah from yadda where blah=foo order by blah) from bar where foo=col order by foo) from table where col = ? order by col");
			Assert.AreEqual(" from table where col = ? ", sql.GetSubselectString().ToString());
		}
		...
		{
			SqlString sql = SqlString.Parse("select (select foo from bar where foo=col order by foo), (select foo from bar where foo=col order by foo) from table where col = ? order by col");
			Assert.AreEqual(" from table where col = ? ", sql.GetSubselectString().ToString());
		}
