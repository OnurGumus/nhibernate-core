...
namespace NHibernate.Test.DialectTest
{
	...
	public partial class MsSqlDialectFixtureAsync : DialectFixtureAsync
	{
		[SetUp]
		...
		public void GetLimitString()
		{
			...
			Assert.AreEqual("select top 100 id, col1, col2 from someTable", limitedSql.ToString(), "Bad limit SQL");
			...
			limitedSql = d.GetLimitString(builder.ToSqlString(), null, new SqlString("100"));
			Assert.AreEqual("select top 100 id, col1, col2 from someTable where id in (select id from othertable)", limitedSql.ToString(), "Bad limit SQL");
			// now the distinct case
			// first with simple string
			builder = new SqlStringBuilder();
			builder.Add("select distinct id, col1, col2 from someTable");
			limitedSql = d.GetLimitString(builder.ToSqlString(), null, new SqlString("100"));
			Assert.AreEqual("select distinct top 100 id, col1, col2 from someTable", limitedSql.ToString(), "Bad limit SQL");
			// now with parts
			builder = new SqlStringBuilder();
