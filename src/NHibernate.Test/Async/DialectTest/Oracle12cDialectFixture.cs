...
namespace NHibernate.Test.DialectTest
{
	...
	public partial class oracle12cDialectFixtureAsync
	{
		[Test]
		public void GetLimitString()
		{
			...
			Assert.AreEqual("SELECT fish.id FROM fish OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
			str = d.GetLimitString(new SqlString("SELECT DISTINCT fish_.id FROM fish fish_"), new SqlString("111"), new SqlString("222"));
			Assert.AreEqual("SELECT DISTINCT fish_.id FROM fish fish_ OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
			str = d.GetLimitString(new SqlString("SELECT DISTINCT fish_.id as ixx9_ FROM fish fish_"), new SqlString("111"), new SqlString("222"));
			Assert.AreEqual("SELECT DISTINCT fish_.id as ixx9_ FROM fish fish_ OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
			...
			Assert.AreEqual("SELECT *, (SELECT COUNT(1) FROM fowl WHERE fish_id = fish.id) AS some_count FROM fish OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
			...
			Assert.AreEqual("SELECT * FROM fish WHERE scales = ? OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
		...
		public void GetLimitStringWithInnerOrder()
		{
			...
			Assert.AreEqual("SELECT * FROM A LEFT JOIN (SELECT top 7 * FROM B ORDER BY name) AS B on A.Name = B.Name OFFSET 111 ROWS FETCH FIRST 222 ROWS ONLY", str.ToString());
		}
