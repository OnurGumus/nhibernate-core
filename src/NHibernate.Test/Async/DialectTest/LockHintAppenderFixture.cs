...
namespace NHibernate.Test.DialectTest
{
	...
	public partial class LockHintAppenderFixtureAsync
	{
		...
		{
			const string expectedQuery = "select * from Person person with (updlock, rowlock) inner join Country country on person.Id = country.Id";
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, CONCAT(FirstName, LastName) from Employee with (updlock, rowlock) union all select Id, CONCAT(FirstName, LastName) from Manager with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, Name FROM dbo.Employee with (updlock, rowlock) union all select Id, Name from Manager with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, Name FROM nhibernate.dbo.Employee with (updlock, rowlock) union all select Id, Name from Manager with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, Name FROM nhibernate..Employee with (updlock, rowlock) union all select Id, Name from Manager with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, Name from [Empl oyee] with (updlock, rowlock) union all select Id, Name from [Man ager] with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
		...
		{
			const string expectedQuery = "select Id, Name from (select Id, Name from [Empl ]]oyee] with (updlock, rowlock) union all select Id, Name from [Manager] with (updlock, rowlock)) as person";
			var result = _appender.AppendLockHint(new SqlString(expectedQuery.Replace(MsSql2000LockHint, string.Empty)));
			Assert.That(result.ToString(), Is.EqualTo(expectedQuery));
		}
