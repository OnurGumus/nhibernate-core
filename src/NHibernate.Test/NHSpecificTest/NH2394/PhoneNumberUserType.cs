...
namespace NHibernate.Test.NHSpecificTest.NH2394
{
	class PhoneNumberUserType : ICompositeUserType
	{
		...
		public async Task NullSafeSet(IDbCommand cmd, object value, int index, bool[] settable, ISessionImplementor session)
		{
			...
			if (settable[0]) await NHibernateUtil.Int32.NullSafeSet(cmd, countryCode, index++, session).ConfigureAwait(false);
			if (settable[1]) await NHibernateUtil.String.NullSafeSet(cmd, number, index, session).ConfigureAwait(false);
		}
